...
Module ParserFn (Import D : Defs.T).
  ...
  Fixpoint multistep (gr : grammar)
    ...
    | StepK ps' ss' ts' av' un' ca' =>
      fun hs => multistep gr pm hp cm ps' ss' ts' av' un' ca'
                          (step_preserves_cache_invar _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ hc hs)
                          (StepK_result_acc _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ ha hc hs)
    end eq_refl.
  ...
  Lemma empty_cache_stores_target_results :
    forall gr pm hp cm,
      cache_stores_target_results gr pm hp cm empty_cache.
  Proof.
    intros gr pm hp cm sps a sps' hf.
  ...
  Definition parse (g : grammar) : nonterminal -> list token -> parse_result :=
    ...
    let hp := mkProductionMap_correct g in
    let cm := mkClosureMap g            in
    fun (x : nonterminal) (ts : list token) =>
      let p_stk0 := (PF []   []    , []) in
      let s_stk0 := (SF None [NT x], []) in
      multistep g pm hp cm p_stk0 s_stk0 ts (allNts g) true empty_cache
                (empty_cache_stores_target_results g pm hp cm) (lex_nat_triple_wf _).
