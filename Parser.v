...
Module ParserFn (Import D : Defs.T).
  ...
  Module Export PredictionComplete := PredictionCompleteFn D.
  ...
  Definition step_opt (g      : grammar)
    ...
    | (PF pre v, p_frs), (SF suf, s_frs) =>
      ...
      | NT x :: _ => 
        if NtSet.mem x av then
          match adaptivePredict g x s_stk ts c with
          | (c', PredSucc rhs) =>
            let p_stk' := (PF [] [], PF pre v :: p_frs) in
            let s_stk' := (SF rhs, SF suf :: s_frs)     in
  ...
  Lemma StepK_result_acc_opt :
    forall g ps ps' ss ss' ts ts' av av' u u' c c' (a : Acc lex_nat_triple (meas g ss ts av)),
      step_opt g ps ss ts av u c = StepK_opt ps' ss' ts' av' u' c'
      -> Acc lex_nat_triple (meas g ss' ts' av').
  Proof.
    intros; eapply Acc_inv; eauto.
    apply magic.
  ...
  Fixpoint multistep_opt (g  : grammar)
    ...
    match step_opt g ps ss ts av u c as res return step_opt g ps ss ts av u c = res -> _ with
    ...
    | StepK_opt ps' ss' ts' av' u' c' =>
      fun hs => multistep_opt g ps' ss' ts' av' u' c'
                              (StepK_result_acc_opt _ _ _ _ _ _ _ _ _ _ _ _ _ a hs)
    end eq_refl.
  
  Definition parse_opt (g : grammar) (gamma : list symbol) (ts : list token) : parse_result :=
    let p_stk0 := (PF [] [], []) in
    let s_stk0 := (SF gamma, []) in
    multistep_opt g p_stk0 s_stk0 ts (allNts g) true (Cache.empty (list subparser)) (lex_nat_triple_wf _). 
