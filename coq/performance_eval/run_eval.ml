...
let chars_of_int i = chars_of_string (string_of_int i)
...
let run_gparser_trial (fname : string) : float * float =
  ...
  let (parsetime, v) = benchmark (parse_opt jsonGrammar [NT value]) ts' in
...
let compare_semantic_values (fname : string) : unit =
   ...
   *)
let () =
  let (lextime, parsetime) = run_gparser_trial "data/nobel_01000.json" in
  let _ = print_float lextime in
  let _ = print_string "\n" in
  let _ = print_float parsetime in
  let _ = print_newline
  in ()
