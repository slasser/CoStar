...
static PyOS_sighandler_t old_siginthandler = (void (*)(int))0;
...
static char default_int_handler_doc[] = "default_int_handler(...)\n\nThe default handler for SIGINT installed by Python.\nIt raises KeyboardInterrupt.";
...
trip_signal(int sig_num)
{
    ...
    is_tripped = 1;
    Py_AddPendingCall(checksignals_witharg, ((void *)0));
}
...
static char alarm_doc[] = "alarm(seconds)\n\nArrange for SIGALRM to arrive after the given number of seconds.";
...
"signal handler must be signal.SIG_IGN, signal.SIG_DFL, or a callable object");
        ...
        func = signal_handler;
    if (PyOS_setsig(sig_num, func) == ((void (*)(int))-1)) {
...
signal_getsignal(PyObject *self, PyObject *args)
{
    ...
    old_handler = Handlers[sig_num].func;
    ( ((PyObject*)(old_handler))->ob_refcnt++);
    return old_handler;
}

static char getsignal_doc[] = "getsignal(sig) -> action\n\nReturn the current action for the given signal.  The return value can be:\nSIG_IGN -- if the signal is being ignored\nSIG_DFL -- if the default action for the signal is in effect\nNone -- if an unknown handler is in effect\nanything else -- the callable Python object used as a handler";
# 389 "signalmodule.c"
static char siginterrupt_doc[] = "siginterrupt(sig, flag) -> None\nchange system call restart behaviour: if flag is False, system calls\nwill be restarted when interrupted by signal sig, else system calls\nwill be interrupted.";
...
static char set_wakeup_fd_doc[] = "set_wakeup_fd(fd) -> fd\n\nSets the fd to be written to (with '\\0') when a signal\ncomes in.  A library can use this to wakeup select or poll.\nThe previous fd is returned.\n\nThe fd must be non-blocking.";
# 452 "signalmodule.c"
int
...
static char setitimer_doc[] = "setitimer(which, seconds[, interval])\n\nSets given itimer (one of ITIMER_REAL, ITIMER_VIRTUAL\nor ITIMER_PROF) to fire after value seconds and after\nthat every interval seconds.\nThe itimer can be cleared by setting seconds to zero.\n\nReturns old values as a tuple: (delay, interval).";
# 499 "signalmodule.c"
static PyObject *
...
static char getitimer_doc[] = "getitimer(which)\n\nReturns current value of given itimer.";
# 527 "signalmodule.c"
static int
iterable_to_sigset(PyObject *iterable, sigset_t *mask)
{
    ...
    while (1)
    {
        ...
        if (0 < signum && signum < 32)
            err = (*(mask) |= __sigbits((int)signum), 0);
        else
            err = 1;
        if (err) {
...
error:
    do { if ((iterator) == ((void *)0)) ; else do { if ( --((PyObject*)(iterator))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(iterator)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(iterator)))); } while (0); } while (0);
    return result;
}
...
sigset_to_set(sigset_t mask)
{
    ...
    for (sig = 1; sig < 32; sig++) {
        ...
        if (PySet_Add(result, signum) == -1) {
            do { if ( --((PyObject*)(signum))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(signum)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(signum)))); } while (0);
            do { if ( --((PyObject*)(result))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(result)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(result)))); } while (0);
            return ((void *)0);
        }
        do { if ( --((PyObject*)(signum))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(signum)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(signum)))); } while (0);
...
static char signal_pthread_sigmask_doc[] = "pthread_sigmask(how, mask) -> old mask\n\nFetch and/or change the signal mask of the calling thread.";
...
static char signal_sigpending_doc[] = "sigpending() -> list\n\nExamine pending signals.";
...
static char signal_sigwait_doc[] = "sigwait(sigset) -> signum\n\nWait a signal.";
# 826 "signalmodule.c"
static PyObject *
...
static char signal_pthread_kill_doc[] = "pthread_kill(thread_id, signum)\n\nSend a signal to a thread.";
# 859 "signalmodule.c"
static PyMethodDef signal_methods[] = {

    {"alarm", signal_alarm, 0x0001, alarm_doc},


    {"setitimer", signal_setitimer, 0x0001, setitimer_doc},


    {"getitimer", signal_getitimer, 0x0001, getitimer_doc},

    {"signal", signal_signal, 0x0001, signal_doc},
    {"getsignal", signal_getsignal, 0x0001, getsignal_doc},
    {"set_wakeup_fd", signal_set_wakeup_fd, 0x0001, set_wakeup_fd_doc},

    {"siginterrupt", signal_siginterrupt, 0x0001, siginterrupt_doc},
...
PyInit_signal(void)
{
    ...
    m = PyModule_Create2(&signalmodule, 1013);
    if (m == ((void *)0))
        return ((void *)0);
# 983 "signalmodule.c"
    d = PyModule_GetDict(m);

    x = DefaultHandler = PyLong_FromVoidPtr((void *)(void (*)(int))0);
    if (!x || PyDict_SetItemString(d, "SIG_DFL", x) < 0)
        goto finally;

    x = IgnoreHandler = PyLong_FromVoidPtr((void *)(void (*)(int))1);
    if (!x || PyDict_SetItemString(d, "SIG_IGN", x) < 0)
        goto finally;

    x = PyLong_FromLong((long)32);
    if (!x || PyDict_SetItemString(d, "NSIG", x) < 0)
        goto finally;
    do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0);


    if (PyModule_AddIntConstant(m, "SIG_BLOCK", 1))
         goto finally;


    if (PyModule_AddIntConstant(m, "SIG_UNBLOCK", 2))
         goto finally;


    if (PyModule_AddIntConstant(m, "SIG_SETMASK", 3))
         goto finally;
    ...
    for (i = 1; i < 32; i++) {
        ...
        else if (t == (void (*)(int))1)
            Handlers[i].func = IgnoreHandler;
        else
            Handlers[i].func = (&_Py_NoneStruct);
        ( ((PyObject*)(Handlers[i].func))->ob_refcnt++);
    }
    if (Handlers[2].func == DefaultHandler) {

        ( ((PyObject*)(IntHandler))->ob_refcnt++);
        do { if ( --((PyObject*)(Handlers[2].func))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(Handlers[2].func)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(Handlers[2].func)))); } while (0);
        Handlers[2].func = IntHandler;
        old_siginthandler = PyOS_setsig(2, signal_handler);
    }
    ...
    x = PyLong_FromLong(10);
    PyDict_SetItemString(d, "SIGBUS", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(11);
    PyDict_SetItemString(d, "SIGSEGV", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(12);
    PyDict_SetItemString(d, "SIGSYS", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(13);
    PyDict_SetItemString(d, "SIGPIPE", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(14);
    ...
    x = PyLong_FromLong(30);
    PyDict_SetItemString(d, "SIGUSR1", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(31);
    ...
    x = PyLong_FromLong(23);
    PyDict_SetItemString(d, "SIGIO", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(16);
    PyDict_SetItemString(d, "SIGURG", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(28);
    ...
    x = PyLong_FromLong(17);
    PyDict_SetItemString(d, "SIGSTOP", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(18);
    PyDict_SetItemString(d, "SIGTSTP", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(19);
    PyDict_SetItemString(d, "SIGCONT", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(21);
    PyDict_SetItemString(d, "SIGTTIN", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(22);
    PyDict_SetItemString(d, "SIGTTOU", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(26);
    PyDict_SetItemString(d, "SIGVTALRM", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(27);
    PyDict_SetItemString(d, "SIGPROF", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(24);
    PyDict_SetItemString(d, "SIGXCPU", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);


    x = PyLong_FromLong(25);
    PyDict_SetItemString(d, "SIGXFSZ", x);
    do { if ((x) == ((void *)0)) ; else do { if ( --((PyObject*)(x))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(x)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(x)))); } while (0); } while (0);
# 1223 "signalmodule.c"
    x = PyLong_FromLong(29);
    ...
    ItimerError = PyErr_NewException("signal.ItimerError",
     PyExc_IOError, ((void *)0));
    if (ItimerError != ((void *)0))
...
finisignal(void)
{
    ...
    PyOS_setsig(2, old_siginthandler);
    old_siginthandler = (void (*)(int))0;

    for (i = 1; i < 32; i++) {
        func = Handlers[i].func;
        Handlers[i].tripped = 0;
        Handlers[i].func = ((void *)0);
        if (i != 2 && func != ((void *)0) && func != (&_Py_NoneStruct) &&
            func != DefaultHandler && func != IgnoreHandler)
            PyOS_setsig(i, (void (*)(int))0);
    ...
    do { if ((IntHandler) == ((void *)0)) ; else do { if ( --((PyObject*)(IntHandler))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(IntHandler)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(IntHandler)))); } while (0); } while (0);
    IntHandler = ((void *)0);
    do { if ((DefaultHandler) == ((void *)0)) ; else do { if ( --((PyObject*)(DefaultHandler))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(DefaultHandler)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(DefaultHandler)))); } while (0); } while (0);
    DefaultHandler = ((void *)0);
    do { if ((IgnoreHandler) == ((void *)0)) ; else do { if ( --((PyObject*)(IgnoreHandler))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(IgnoreHandler)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(IgnoreHandler)))); } while (0); } while (0);
    IgnoreHandler = ((void *)0);
}
...
# 1334 "signalmodule.c"
    ...
    for (i = 1; i < 32; i++) {
        if (Handlers[i].tripped) {
            ...
            if (arglist) {
                result = PyEval_CallObjectWithKeywords(Handlers[i].func, arglist, (PyObject *)((void *)0));
...
{
    if (Handlers[2].tripped) {

        if (PyThread_get_thread_ident() != main_thread)
            return 0;

        Handlers[2].tripped = 0;
        return 1;
    }
    return 0;
}
