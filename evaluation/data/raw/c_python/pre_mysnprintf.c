...
PyOS_vsnprintf(char *str, size_t size, const char *format, va_list va)
{
    ...
    (__builtin_expect(!(str != ((void *)0)), 0) ? __assert_rtn(__func__, "mysnprintf.c", 62, "str != NULL") : (void)0);
    (__builtin_expect(!(size > 0), 0) ? __assert_rtn(__func__, "mysnprintf.c", 63, "size > 0") : (void)0);
    (__builtin_expect(!(format != ((void *)0)), 0) ? __assert_rtn(__func__, "mysnprintf.c", 64, "format != NULL") : (void)0);



    if (size > 2147483647 - 1) {
        len = -666;
        goto Done;
    }


    len = __builtin___vsnprintf_chk (str, size, 0, __builtin_object_size (str, 2 > 1), format, va);
# 99 "mysnprintf.c"
Done:
    if (size > 0)
        str[size-1] = '\0';
