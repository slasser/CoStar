...
# 10 "_freeze_importlib.c" 2
# 20 "_freeze_importlib.c"
...
main(int argc, char *argv[])
{
    ...
    if (argc != 3) {
        fprintf(__stderrp, "need to specify input and output paths\n");
        return 2;
    }
    inpath = argv[1];
    outpath = argv[2];
    ...
    if (fstat(fileno(infile), &st)) {
        fclose(infile);
        fprintf(__stderrp, "cannot fstat '%s'\n", inpath);
        return 1;
    }
    ...
    if (text == ((void *)0)) {
        fclose(infile);
        fprintf(__stderrp, "could not allocate %ld bytes\n", (long) text_size);
        return 1;
    }
    ...
    infile = ((void *)0);
    if (n < text_size) {
        fprintf(__stderrp, "read too short: got %ld instead of %ld bytes\n",
    ...
    if (code == ((void *)0))
        goto error;
    marshalled = PyMarshal_WriteObjectToString(code, 2);
    do { if ( --((PyObject*)(code))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(code)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(code)))); } while (0);
    if (marshalled == ((void *)0))
        goto error;

    (__builtin_expect(!(((((PyObject*)(marshalled))->ob_type) == &PyBytes_Type)), 0) ? __assert_rtn(__func__, "_freeze_importlib.c", 96, "PyBytes_CheckExact(marshalled)") : (void)0);
    data = (unsigned char *) ((__builtin_expect(!(((((((PyObject*)(marshalled))->ob_type))->tp_flags & ((1L<<27))) != 0)), 0) ? __assert_rtn(__func__, "_freeze_importlib.c", 97, "PyBytes_Check(marshalled)") : (void)0), (((PyBytesObject *)(marshalled))->ob_sval));
    data_size = ((__builtin_expect(!(((((((PyObject*)(marshalled))->ob_type))->tp_flags & ((1L<<27))) != 0)), 0) ? __assert_rtn(__func__, "_freeze_importlib.c", 98, "PyBytes_Check(marshalled)") : (void)0),(((PyVarObject*)(marshalled))->ob_size));



    outfile = fopen(outpath, "w");
    if (outfile == ((void *)0)) {
        fprintf(__stderrp, "cannot open '%s' for writing\n", outpath);
        return 1;
    }
    fprintf(outfile, "%s\n", header);
    fprintf(outfile, "unsigned char _Py_M__importlib[] = {\n");
    for (n = 0; n < data_size; n += 16) {
        size_t i, end = (((n + 16) > (data_size)) ? (data_size) : (n + 16));
        fprintf(outfile, "    ");
        for (i = n; i < end; i++) {
    ...
    do { if ( --((PyObject*)(marshalled))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(marshalled)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(marshalled)))); } while (0);
    ...
    if (outfile) {
        if (ferror(outfile)) {
            fprintf(__stderrp, "error when writing to '%s'\n", outpath);
            fclose(outfile);
            return 1;
        }
        fclose(outfile);
