...
# 27 "myreadline.c" 2
static PyThread_type_lock _PyOS_ReadlineLock = ((void *)0);


int (*PyOS_InputHook)(void) = ((void *)0);
...
PyOS_StdioReadline(FILE *sys_stdin, FILE *sys_stdout, char *prompt)
{
    ...
    char *p;
    n = 100;
    if ((p = (char *)((size_t)(n) > (size_t)((Py_ssize_t)(((size_t)-1)>>1)) ? ((void *)0) : malloc((n) ? (n) : 1))) == ((void *)0))
        return ((void *)0);
    fflush(sys_stdout);
    if (prompt)
    ...
    while (n > 0 && p[n-1] != '\n') {
        size_t incr = n+2;
        p = (char *)((size_t)(n + incr) > (size_t)((Py_ssize_t)(((size_t)-1)>>1)) ? ((void *)0) : realloc((p), (n + incr) ? (n + incr) : 1));
        if (p == ((void *)0))
            return ((void *)0);
        if (incr > 2147483647) {
...
PyOS_Readline(FILE *sys_stdin, FILE *sys_stdout, char *prompt)
{
    ...
    if (PyOS_ReadlineFunctionPointer == ((void *)0)) {
    ...
    if (_PyOS_ReadlineLock == ((void *)0)) {
        _PyOS_ReadlineLock = PyThread_allocate_lock();
    }


    _PyOS_ReadlineTState = ((PyThreadState*)__extension__ ({ __typeof__(&_PyThreadState_Current) atomic_val = &_PyThreadState_Current; __typeof__(atomic_val->_value) result; volatile __typeof__(result) *volatile_data = &atomic_val->_value; _Py_memory_order order = _Py_memory_order_relaxed; _Py_ANNOTATE_MEMORY_ORDER(atomic_val, order); ; switch(order) { case _Py_memory_order_release: case _Py_memory_order_acq_rel: case _Py_memory_order_seq_cst: _Py_atomic_thread_fence(_Py_memory_order_release); break; default: break; } result = *volatile_data; switch(order) { case _Py_memory_order_acquire: case _Py_memory_order_acq_rel: case _Py_memory_order_seq_cst: _Py_atomic_signal_fence(_Py_memory_order_acquire); break; default: break; } ; result; }));
