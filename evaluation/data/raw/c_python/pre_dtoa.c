...
multadd(Bigint *b, int m, int a)
{
    ...
    if (carry) {
        if (wds >= b->maxwds) {
            ...
            ((__builtin_object_size ((char *)&b1->sign, 0) != (size_t) -1) ? __builtin___memcpy_chk ((char *)&b1->sign, (char *)&b->sign, b->wds*sizeof(Long) + 2*sizeof(int), __builtin_object_size ((char *)&b1->sign, 0)) : __inline_memcpy_chk ((char *)&b1->sign, (char *)&b->sign, b->wds*sizeof(Long) + 2*sizeof(int)));
            Bfree(b);
            b = b1;
        }
        b->x[wds++] = (ULong)carry;
...
sd2b(U *d, int scale, int *e)
{
    ...
    b->x[0] = (d)->L[0];
    b->x[1] = (d)->L[1] & 0xfffff;
    *e = (-1074) - 1 + (int)(((d)->L[1] & 0x7ff00000) >> 20);
    if (*e < (-1074))
        *e = (-1074);
    else
        b->x[1] |= 0x100000;
        ...
        *e -= scale;
        if (*e < (-1074)) {
            ...
            *e = (-1074);
            ...
            (__builtin_expect(!(0 < scale && scale <= 53 - 1), 0) ? __assert_rtn(__func__, "dtoa.c", 1091, "0 < scale && scale <= P - 1") : (void)0);
            if (scale >= 32) {

                (__builtin_expect(!(b->x[0] == 0), 0) ? __assert_rtn(__func__, "dtoa.c", 1094, "b->x[0] == 0") : (void)0);
                b->x[0] = b->x[1];
                b->x[1] = 0;
                scale -= 32;
            }
            if (scale) {
                ...
                (__builtin_expect(!(b->x[0] << (32 - scale) == 0), 0) ? __assert_rtn(__func__, "dtoa.c", 1101, "b->x[0] << (32 - scale) == 0") : (void)0);
...
sulp(U *x, BCinfo *bc)
{
    ...
    if (bc->scale && 2*53 + 1 > (int)(((x)->L[1] & 0x7ff00000) >> 20)) {
        ...
        (__builtin_expect(!((x)->L[1] || (x)->L[0]), 0) ? __assert_rtn(__func__, "dtoa.c", 1338, "word0(x) || word1(x)") : (void)0);
        return ulp(x);
    }
}
# 1389 "dtoa.c"
...
# 1887 "dtoa.c"
        ...
        ((__builtin_object_size ((char *)&bd->sign, 0) != (size_t) -1) ? __builtin___memcpy_chk ((char *)&bd->sign, (char *)&bd0->sign, bd0->wds*sizeof(Long) + 2*sizeof(int), __builtin_object_size ((char *)&bd->sign, 0)) : __inline_memcpy_chk ((char *)&bd->sign, (char *)&bd0->sign, bd0->wds*sizeof(Long) + 2*sizeof(int)));
...
# 1946 "dtoa.c"
        ...
        if (bc.nd > nd && i <= 0) {
            if (dsign)
                break;
# 2041 "dtoa.c"
            if (!(&rv)->L[0] && !((&rv)->L[1] & 0xfffff)) {


                j = (int)((&rv)->L[1] & 0x7ff00000) >> 20;
        ...
        if (i == 0) {
            ...
            if (dsign) {
                if (((&rv)->L[1] & 0xfffff) == 0xfffff
                    && (&rv)->L[0] == (
                        (bc.scale &&
                         (y = (&rv)->L[1] & 0x7ff00000) <= 2*53*0x100000) ?
...
# 2756 "dtoa.c"
    ...
    if (leftright) {
        ...
        if (spec_case) {
            ...
            if (mhi == ((void *)0))
                goto failed_malloc;
            ((__builtin_object_size ((char *)&mhi->sign, 0) != (size_t) -1) ? __builtin___memcpy_chk ((char *)&mhi->sign, (char *)&mlo->sign, mlo->wds*sizeof(Long) + 2*sizeof(int), __builtin_object_size ((char *)&mhi->sign, 0)) : __inline_memcpy_chk ((char *)&mhi->sign, (char *)&mlo->sign, mlo->wds*sizeof(Long) + 2*sizeof(int)));
            mhi = lshift(mhi, 1);
            if (mhi == ((void *)0))
                goto failed_malloc;
        }
