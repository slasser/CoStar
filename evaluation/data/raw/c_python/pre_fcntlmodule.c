...
fcntl_fcntl(PyObject *self, PyObject *args)
{
    ...
    if (_PyArg_ParseTuple_SizeT(args, "O&is#:fcntl",
        ...
        ((__builtin_object_size (buf, 0) != (size_t) -1) ? __builtin___memcpy_chk (buf, str, len, __builtin_object_size (buf, 0)) : __inline_memcpy_chk (buf, str, len));
...
# 113 "fcntlmodule.c"
    ...
    if (_PyArg_ParseTuple_SizeT(args, "O&Iw*|i:ioctl",
        ...
        if (mutate_arg && (len <= 1024)) {
            ((__builtin_object_size (str, 0) != (size_t) -1) ? __builtin___memcpy_chk (str, buf, len, __builtin_object_size (str, 0)) : __inline_memcpy_chk (str, buf, len));
        }
        PyBuffer_Release(&pstr);
        if (ret < 0) {
            PyErr_SetFromErrno(PyExc_IOError);
    ...
    PyErr_Clear();
    if (_PyArg_ParseTuple_SizeT(args, "O&Is*:ioctl",
...
fcntl_flock(PyObject *self, PyObject *args)
{
    ...
    if (!_PyArg_ParseTuple_SizeT(args, "O&i:flock",
                          conv_descriptor, &fd, &code))
        return ((void *)0);
...
static char flock_doc[] = "flock(fd, operation)\n\nPerform the lock operation op on file descriptor fd.  See the Unix \nmanual page for flock(3) for details.  (On some systems, this function is\nemulated using fcntl().)";
# 317 "fcntlmodule.c"
static PyObject *
fcntl_lockf(PyObject *self, PyObject *args)
{
    int fd, code, ret, whence = 0;
    PyObject *lenobj = ((void *)0), *startobj = ((void *)0);

    if (!_PyArg_ParseTuple_SizeT(args, "O&i|OOi:lockf",
                          conv_descriptor, &fd, &code,
...
static char lockf_doc[] = "lockf (fd, operation, length=0, start=0, whence=0)\n\nThis is essentially a wrapper around the fcntl() locking calls.  fd is the\nfile descriptor of the file to lock or unlock, and operation is one of the\nfollowing values:\n\n    LOCK_UN - unlock\n    LOCK_SH - acquire a shared lock\n    LOCK_EX - acquire an exclusive lock\n\nWhen operation is LOCK_SH or LOCK_EX, it can also be bitwise ORed with\nLOCK_NB to avoid blocking on lock acquisition.  If LOCK_NB is used and the\nlock cannot be acquired, an IOError will be raised and the exception will\nhave an errno attribute set to EACCES or EAGAIN (depending on the operating\nsystem -- for portability, check for either value).\n\nlength is the number of bytes to lock, with the default meaning to lock to\nEOF.  start is the byte offset, relative to whence, to that the lock\nstarts.  whence is as with fileobj.seek(), specifically:\n\n    0 - relative to the start of the file (SEEK_SET)\n    1 - relative to the current buffer position (SEEK_CUR)\n    2 - relative to the end of the file (SEEK_END)";
# 416 "fcntlmodule.c"
static PyMethodDef fcntl_methods[] = {
    {"fcntl", fcntl_fcntl, 0x0001, fcntl_doc},
    {"ioctl", fcntl_ioctl, 0x0001, ioctl_doc},
    {"flock", fcntl_flock, 0x0001, flock_doc},
    {"lockf", fcntl_lockf, 0x0001, lockf_doc},
    {((void *)0), ((void *)0)}
};


static char module_doc[] = "This module performs file control and I/O control on file \ndescriptors.  It is an interface to the fcntl() and ioctl() Unix\nroutines.  File descriptors can be obtained with the fileno() method of\na file or socket object.";
...
all_ins(PyObject* d)
{
    if (ins(d, "LOCK_SH", (long)0x01)) return -1;
    if (ins(d, "LOCK_EX", (long)0x02)) return -1;
    if (ins(d, "LOCK_NB", (long)0x04)) return -1;
    if (ins(d, "LOCK_UN", (long)0x08)) return -1;
# 468 "fcntlmodule.c"
    if (ins(d, "F_DUPFD", (long)0)) return -1;


    if (ins(d, "F_GETFD", (long)1)) return -1;


    if (ins(d, "F_SETFD", (long)2)) return -1;


    if (ins(d, "F_GETFL", (long)3)) return -1;


    if (ins(d, "F_SETFL", (long)4)) return -1;


    if (ins(d, "F_GETLK", (long)7)) return -1;


    if (ins(d, "F_SETLK", (long)8)) return -1;


    if (ins(d, "F_SETLKW", (long)9)) return -1;


    if (ins(d, "F_GETOWN", (long)5)) return -1;


    if (ins(d, "F_SETOWN", (long)6)) return -1;
# 504 "fcntlmodule.c"
    if (ins(d, "F_RDLCK", (long)1)) return -1;


    if (ins(d, "F_WRLCK", (long)3)) return -1;


    if (ins(d, "F_UNLCK", (long)2)) return -1;
# 524 "fcntlmodule.c"
    if (ins(d, "FASYNC", (long)0x0040)) return -1;
# 545 "fcntlmodule.c"
    if (ins(d, "F_FULLFSYNC", (long)51)) return -1;


    if (ins(d, "F_NOCACHE", (long)48)) return -1;




    if (ins(d, "FD_CLOEXEC", (long)1)) return -1;
# 617 "fcntlmodule.c"
    return 0;
}
...
PyInit_fcntl(void)
{
    ...
    m = PyModule_Create2(&fcntlmodule, 1013);
    if (m == ((void *)0))
        return ((void *)0);
