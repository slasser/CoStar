...
PyOS_mystrnicmp(const char *s1, const char *s2, Py_ssize_t size)
{
    if (size == 0)
        return 0;
    while ((--size > 0) &&
           (towlower(btowc((unsigned)*s1)) == towlower(btowc((unsigned)*s2)))) {
        if (!*s1++ || !*s2++)
            break;
    }
    return towlower(btowc((unsigned)*s1)) - towlower(btowc((unsigned)*s2));
}

int
PyOS_mystricmp(const char *s1, const char *s2)
{
    while (*s1 && (towlower(btowc((unsigned)*s1++)) == towlower(btowc((unsigned)*s2++)))) {
        ;
    }
    return (towlower(btowc((unsigned)*s1)) - towlower(btowc((unsigned)*s2)));
}
