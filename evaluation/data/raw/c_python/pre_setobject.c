...
static PyObject *dummy = ((void *)0);
# 58 "setobject.c"
static PySetObject *free_list[80];
...
set_lookkey(PySetObject *so, PyObject *key, register Py_hash_t hash)
{
    ...
    i = (size_t)hash & mask;
    entry = &table[i];
    if (entry->key == ((void *)0) || entry->key == key)
        return entry;
    ...
    for (perturb = hash; ; perturb >>= 5) {
        ...
        else if (entry->key == dummy && freeslot == ((void *)0))
            freeslot = entry;
    }
    return entry;
}
...
set_lookkey_unicode(PySetObject *so, PyObject *key, register Py_hash_t hash)
{
    ...
    for (perturb = hash; ; perturb >>= 5) {
        ...
        entry = &table[i & mask];
        if (entry->key == ((void *)0))
            return freeslot == ((void *)0) ? entry : freeslot;
        if (entry->key == key
            || (entry->hash == hash
            && entry->key != dummy
            && unicode_eq(entry->key, key)))
            return entry;
        if (entry->key == dummy && freeslot == ((void *)0))
            freeslot = entry;
    }
    (__builtin_expect(!(0), 0) ? __assert_rtn(__func__, "setobject.c", 204, "0") : (void)0);
    return 0;
}
...
set_insert_key(register PySetObject *so, PyObject *key, Py_hash_t hash)
{
    ...
    (__builtin_expect(!(so->lookup != ((void *)0)), 0) ? __assert_rtn(__func__, "setobject.c", 219, "so->lookup != NULL") : (void)0);
    ...
    if (entry->key == ((void *)0)) {
        ...
        entry->hash = hash;
        so->used++;
        do { if ( --((PyObject*)(dummy))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(dummy)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(dummy)))); } while (0);
    } else {
...
set_insert_clean(register PySetObject *so, PyObject *key, Py_hash_t hash)
{
    ...
    i = (size_t)hash & mask;
    entry = &table[i];
    for (perturb = hash; entry->key != ((void *)0); perturb >>= 5) {
        i = (i << 2) + i + perturb + 1;
        entry = &table[i & mask];
    }
    so->fill++;
...
set_table_resize(PySetObject *so, Py_ssize_t minused)
{
    ...
    (__builtin_expect(!(minused >= 0), 0) ? __assert_rtn(__func__, "setobject.c", 285, "minused >= 0") : (void)0);
    ...
    oldtable = so->table;
    (__builtin_expect(!(oldtable != ((void *)0)), 0) ? __assert_rtn(__func__, "setobject.c", 299, "oldtable != NULL") : (void)0);
    ...
    if (newsize == 8) {
        ...
        if (newtable == oldtable) {
            ...
            (__builtin_expect(!(so->fill > so->used), 0) ? __assert_rtn(__func__, "setobject.c", 316, "so->fill > so->used") : (void)0);
            ((__builtin_object_size (small_copy, 0) != (size_t) -1) ? __builtin___memcpy_chk (small_copy, oldtable, sizeof(small_copy), __builtin_object_size (small_copy, 0)) : __inline_memcpy_chk (small_copy, oldtable, sizeof(small_copy)));
            oldtable = small_copy;
        }
    }
    else {
        newtable = ( ((size_t)(newsize) > ((Py_ssize_t)(((size_t)-1)>>1)) / sizeof(setentry)) ? ((void *)0) : ( (setentry *) ((size_t)((newsize) * sizeof(setentry)) > (size_t)((Py_ssize_t)(((size_t)-1)>>1)) ? ((void *)0) : malloc(((newsize) * sizeof(setentry)) ? ((newsize) * sizeof(setentry)) : 1)) ) );
        if (newtable == ((void *)0)) {
            PyErr_NoMemory();
            return -1;
        }
    ...
    (__builtin_expect(!(newtable != oldtable), 0) ? __assert_rtn(__func__, "setobject.c", 330, "newtable != oldtable") : (void)0);
    so->table = newtable;
    so->mask = newsize - 1;
    ((__builtin_object_size (newtable, 0) != (size_t) -1) ? __builtin___memset_chk (newtable, 0, sizeof(setentry) * newsize, __builtin_object_size (newtable, 0)) : __inline_memset_chk (newtable, 0, sizeof(setentry) * newsize));
    so->used = 0;
    i = so->fill;
    so->fill = 0;
    ...
    for (entry = oldtable; i > 0; entry++) {
        if (entry->key == ((void *)0)) {
            ...
            --i;
            (__builtin_expect(!(entry->key == dummy), 0) ? __assert_rtn(__func__, "setobject.c", 347, "entry->key == dummy") : (void)0);
            do { if ( --((PyObject*)(entry->key))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(entry->key)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(entry->key)))); } while (0);
        } else {
...
set_add_entry(register PySetObject *so, setentry *entry)
{
    ...
    (__builtin_expect(!(so->fill <= so->mask), 0) ? __assert_rtn(__func__, "setobject.c", 370, "so->fill <= so->mask") : (void)0);
    n_used = so->used;
...
set_add_key(register PySetObject *so, PyObject *key)
{
    ...
    (__builtin_expect(!(so->fill <= so->mask), 0) ? __assert_rtn(__func__, "setobject.c", 394, "so->fill <= so->mask") : (void)0);
    n_used = so->used;
    ( ((PyObject*)(key))->ob_refcnt++);
    if (set_insert_key(so, key, hash) == -1) {
        do { if ( --((PyObject*)(key))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(key)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(key)))); } while (0);
...
{ register setentry *entry;
    ...
    if (entry == ((void *)0))
        return -1;
    if (entry->key == ((void *)0) || entry->key == dummy)
        return 0;
    old_key = entry->key;
    ( ((PyObject*)(dummy))->ob_refcnt++);
    entry->key = dummy;
    so->used--;
...
set_discard_key(PySetObject *so, PyObject *key)
{
    ...
    (__builtin_expect(!(((((PyObject*)(so))->ob_type) == &PySet_Type || (((PyObject*)(so))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PyFrozenSet_Type))), 0) ? __assert_rtn(__func__, "setobject.c", 434, "PyAnySet_Check(so)") : (void)0);
...
# 470 "setobject.c"
    table = so->table;
    (__builtin_expect(!(table != ((void *)0)), 0) ? __assert_rtn(__func__, "setobject.c", 471, "table != NULL") : (void)0);
    ...
    if (table_is_malloced)
        do { ((__builtin_object_size ((so)->smalltable, 0) != (size_t) -1) ? __builtin___memset_chk ((so)->smalltable, 0, sizeof((so)->smalltable), __builtin_object_size ((so)->smalltable, 0)) : __inline_memset_chk ((so)->smalltable, 0, sizeof((so)->smalltable))); (so)->used = (so)->fill = 0; do { (so)->table = (so)->smalltable; (so)->mask = 8 - 1; (so)->hash = -1; } while(0); } while(0);
        ...
        ((__builtin_object_size (small_copy, 0) != (size_t) -1) ? __builtin___memcpy_chk (small_copy, table, sizeof(small_copy), __builtin_object_size (small_copy, 0)) : __inline_memcpy_chk (small_copy, table, sizeof(small_copy)));
        table = small_copy;
        do { ((__builtin_object_size ((so)->smalltable, 0) != (size_t) -1) ? __builtin___memset_chk ((so)->smalltable, 0, sizeof((so)->smalltable), __builtin_object_size ((so)->smalltable, 0)) : __inline_memset_chk ((so)->smalltable, 0, sizeof((so)->smalltable))); (so)->used = (so)->fill = 0; do { (so)->table = (so)->smalltable; (so)->mask = 8 - 1; (so)->hash = -1; } while(0); } while(0);
    }
...
set_next(PySetObject *so, Py_ssize_t *pos_ptr, setentry **entry_ptr)
{
    ...
    (__builtin_expect(!(((((PyObject*)(so))->ob_type) == &PySet_Type || (((PyObject*)(so))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PyFrozenSet_Type))), 0) ? __assert_rtn(__func__, "setobject.c", 539, "PyAnySet_Check(so)") : (void)0);
    i = *pos_ptr;
    (__builtin_expect(!(i >= 0), 0) ? __assert_rtn(__func__, "setobject.c", 541, "i >= 0") : (void)0);
    table = so->table;
    mask = so->mask;
    while (i <= mask && (table[i].key == ((void *)0) || table[i].key == dummy))
        i++;
    *pos_ptr = i+1;
    if (i > mask)
        return 0;
    (__builtin_expect(!(table[i].key != ((void *)0)), 0) ? __assert_rtn(__func__, "setobject.c", 549, "table[i].key != NULL") : (void)0);
    *entry_ptr = &table[i];
    return 1;
}
...
set_dealloc(PySetObject *so)
{
    ...
    if (so->table != so->smalltable)
        free(so->table);
    if (numfree < 80 && ((((PyObject*)(so))->ob_type) == &PySet_Type || (((PyObject*)(so))->ob_type) == &PyFrozenSet_Type))
        free_list[numfree++] = so;
    else
        (((PyObject*)(so))->ob_type)->tp_free(so);
    --_tstate->trash_delete_nesting; if (_tstate->trash_delete_later && _tstate->trash_delete_nesting <= 0) _PyTrash_thread_destroy_chain(); } else _PyTrash_thread_deposit_object((PyObject*)so); } while (0);
}

static PyObject *
set_repr(PySetObject *so)
{
    PyObject *result=((void *)0), *keys, *listrepr, *tmp;
    int status = Py_ReprEnter((PyObject*)so);

    if (status != 0) {
        if (status < 0)
            return ((void *)0);
        return PyUnicode_FromFormat("%s(...)", (((PyObject*)(so))->ob_type)->tp_name);
    }
    ...
    if (listrepr == ((void *)0))
        goto done;
    tmp = PyUnicode_Substring(listrepr, 1, ((__builtin_expect(!(((((((PyObject*)(listrepr))->ob_type))->tp_flags & ((1L<<28))) != 0)), 0) ? __assert_rtn(__func__, "setobject.c", 606, "PyUnicode_Check(listrepr)") : (void)0), (__builtin_expect(!((((PyASCIIObject*)listrepr)->state.ready)), 0) ? __assert_rtn(__func__, "setobject.c", 606, "PyUnicode_IS_READY(listrepr)") : (void)0), ((PyASCIIObject *)(listrepr))->length)-1);
    do { if ( --((PyObject*)(listrepr))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(listrepr)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(listrepr)))); } while (0);
    if (tmp == ((void *)0))
        goto done;
    listrepr = tmp;

    if ((((PyObject*)(so))->ob_type) != &PySet_Type)
        result = PyUnicode_FromFormat("%s({%U})",
                                      (((PyObject*)(so))->ob_type)->tp_name,
                                      listrepr);
    else
...
set_merge(PySetObject *so, PyObject *otherset)
{
    ...
    (__builtin_expect(!(((((PyObject*)(so))->ob_type) == &PySet_Type || (((PyObject*)(so))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PyFrozenSet_Type))), 0) ? __assert_rtn(__func__, "setobject.c", 639, "PyAnySet_Check(so)") : (void)0);
    (__builtin_expect(!(((((PyObject*)(otherset))->ob_type) == &PySet_Type || (((PyObject*)(otherset))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(otherset))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(otherset))->ob_type), &PyFrozenSet_Type))), 0) ? __assert_rtn(__func__, "setobject.c", 640, "PyAnySet_Check(otherset)") : (void)0);
...
set_pop(PySetObject *so)
{
    ...
    (__builtin_expect(!(((((PyObject*)(so))->ob_type) == &PySet_Type || (((PyObject*)(so))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PyFrozenSet_Type))), 0) ? __assert_rtn(__func__, "setobject.c", 709, "PyAnySet_Check(so)") : (void)0);
    if (so->used == 0) {
    ...
    if (entry->key == ((void *)0) || entry->key == dummy) {
        ...
        if (i > so->mask || i < 1)
            i = 1;
        while ((entry = &so->table[i])->key == ((void *)0) || entry->key==dummy) {
            i++;
            if (i > so->mask)
                i = 1;
        }
...
set_traverse(PySetObject *so, visitproc visit, void *arg)
{
    ...
    while (set_next(so, &pos, &entry))
        do { if (entry->key) { int vret = visit((PyObject *)(entry->key), arg); if (vret) return vret; } } while (0);
    return 0;
}
...
frozenset_hash(PyObject *self)
{
    ...
    hash *= (Py_uhash_t)(((PySetObject *)(self))->used) + 1;
    while (set_next(so, &pos, &entry)) {
...
{
    do { if ((si->si_set) == ((void *)0)) ; else do { if ( --((PyObject*)(si->si_set))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(si->si_set)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(si->si_set)))); } while (0); } while (0);
    PyObject_GC_Del(si);
}
...
{
    do { if (si->si_set) { int vret = visit((PyObject *)(si->si_set), arg); if (vret) return vret; } } while (0);
    return 0;
}
...
{
    Py_ssize_t len = 0;
    if (si->si_set != ((void *)0) && si->si_used == si->si_set->used)
        len = si->len;
...
setiter_reduce(setiterobject *si)
{
    ...
    tmp = *si;
    do { if ((tmp.si_set) == ((void *)0)) ; else ( ((PyObject*)(tmp.si_set))->ob_refcnt++); } while (0);
    ...
    for(;;) {
        ...
        if (element) {
            if (PyList_Append(list, element)) {
                ...
                do { if ((tmp.si_set) == ((void *)0)) ; else do { if ( --((PyObject*)(tmp.si_set))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(tmp.si_set)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(tmp.si_set)))); } while (0); } while (0);
                return ((void *)0);
            }
            do { if ( --((PyObject*)(element))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(element)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(element)))); } while (0);
        } else
...
static PyMethodDef setiter_methods[] = {
    {"__length_hint__", (PyCFunction)setiter_len, 0x0004, length_hint_doc},
    {"__reduce__", (PyCFunction)setiter_reduce, 0x0004, reduce_doc},
    {((void *)0), ((void *)0)}
};
...
static PyObject *setiter_iternext(setiterobject *si)
{
    ...
    if (so == ((void *)0))
        return ((void *)0);
    (__builtin_expect(!(((((PyObject*)(so))->ob_type) == &PySet_Type || (((PyObject*)(so))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PyFrozenSet_Type))), 0) ? __assert_rtn(__func__, "setobject.c", 879, "PyAnySet_Check(so)") : (void)0);
    ...
    i = si->si_pos;
    (__builtin_expect(!(i>=0), 0) ? __assert_rtn(__func__, "setobject.c", 889, "i>=0") : (void)0);
    entry = so->table;
    mask = so->mask;
    while (i <= mask && (entry[i].key == ((void *)0) || entry[i].key == dummy))
        i++;
    si->si_pos = i+1;
    if (i > mask)
        goto fail;
...
fail:
    do { if ( --((PyObject*)(so))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(so)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(so)))); } while (0);
    si->si_set = ((void *)0);
    return ((void *)0);
}
...
{
    setiterobject *si = ( (setiterobject *) _PyObject_GC_New(&PySetIter_Type) );
    if (si == ((void *)0))
        return ((void *)0);
    ( ((PyObject*)(so))->ob_refcnt++);
    si->si_set = so;
    si->si_used = so->used;
    si->si_pos = 0;
    si->len = so->used;
    do { PyGC_Head *g = ((PyGC_Head *)(si)-1); if (g->gc.gc_refs != (-2)) Py_FatalError("GC object already tracked"); g->gc.gc_refs = (-3); g->gc.gc_next = _PyGC_generation0; g->gc.gc_prev = _PyGC_generation0->gc.gc_prev; g->gc.gc_prev->gc.gc_next = g; _PyGC_generation0->gc.gc_prev = g; } while (0);;
    return (PyObject *)si;
}
...
set_update_internal(PySetObject *so, PyObject *other)
{
    ...
    if (((((PyObject*)(other))->ob_type) == &PySet_Type || (((PyObject*)(other))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PyFrozenSet_Type)))
        return set_merge(so, other);

    if (((((PyObject*)(other))->ob_type) == &PyDict_Type)) {
        PyObject *value;
        Py_ssize_t pos = 0;
        Py_hash_t hash;
...
set_update(PySetObject *so, PyObject *args)
{
    ...
    for (i=0 ; i<(((PyVarObject*)(args))->ob_size) ; i++) {
        PyObject *other = (((PyTupleObject *)(args))->ob_item[i]);
...
static char update_doc[] = "Update a set with the union of itself and others.";


static PyObject *
make_new_set(PyTypeObject *type, PyObject *iterable)
{
    register PySetObject *so = ((void *)0);
    ...
    if (numfree &&
        ...
        (__builtin_expect(!(so != ((void *)0) && ((((PyObject*)(so))->ob_type) == &PySet_Type || (((PyObject*)(so))->ob_type) == &PyFrozenSet_Type)), 0) ? __assert_rtn(__func__, "setobject.c", 1040, "so != NULL && PyAnySet_CheckExact(so)") : (void)0);
        (((PyObject*)(so))->ob_type) = type;
        ( (((PyObject*)((PyObject *)so))->ob_refcnt) = 1);
        do { ((__builtin_object_size ((so)->smalltable, 0) != (size_t) -1) ? __builtin___memset_chk ((so)->smalltable, 0, sizeof((so)->smalltable), __builtin_object_size ((so)->smalltable, 0)) : __inline_memset_chk ((so)->smalltable, 0, sizeof((so)->smalltable))); (so)->used = (so)->fill = 0; do { (so)->table = (so)->smalltable; (so)->mask = 8 - 1; (so)->hash = -1; } while(0); } while(0);
        ...
        (__builtin_expect(!(so->table == ((void *)0) && so->fill == 0 && so->used == 0), 0) ? __assert_rtn(__func__, "setobject.c", 1050, "so->table == NULL && so->fill == 0 && so->used == 0") : (void)0);
        do { (so)->table = (so)->smalltable; (so)->mask = 8 - 1; (so)->hash = -1; } while(0);
    }

    so->lookup = set_lookkey_unicode;
    so->weakreflist = ((void *)0);
...
static PyObject *emptyfrozenset = ((void *)0);

static PyObject *
frozenset_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    PyObject *iterable = ((void *)0), *result;
    ...
    if (iterable != ((void *)0)) {
        ...
        if (((((PyObject*)(iterable))->ob_type) == &PyFrozenSet_Type)) {
            ( ((PyObject*)(iterable))->ob_refcnt++);
            return iterable;
        }
        result = make_new_set(type, iterable);
        if (result == ((void *)0) || (((PySetObject *)(result))->used))
            return result;
        do { if ( --((PyObject*)(result))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(result)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(result)))); } while (0);
    }

    if (emptyfrozenset == ((void *)0))
        emptyfrozenset = make_new_set(type, ((void *)0));
    do { if ((emptyfrozenset) == ((void *)0)) ; else ( ((PyObject*)(emptyfrozenset))->ob_refcnt++); } while (0);
...
{
    PySet_ClearFreeList();
    do { if (dummy) { PyObject *_py_tmp = (PyObject *)(dummy); (dummy) = ((void *)0); do { if ( --((PyObject*)(_py_tmp))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(_py_tmp)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(_py_tmp)))); } while (0); } } while (0);
    do { if (emptyfrozenset) { PyObject *_py_tmp = (PyObject *)(emptyfrozenset); (emptyfrozenset) = ((void *)0); do { if ( --((PyObject*)(_py_tmp))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(_py_tmp)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(_py_tmp)))); } while (0); } } while (0);
}
...
set_new(PyTypeObject *type, PyObject *args, PyObject *kwds)
{
    ...
    return make_new_set(type, ((void *)0));
}
# 1168 "setobject.c"
static void
set_swap_bodies(PySetObject *a, PySetObject *b)
{
    ...
    if (a->table == a->smalltable || b->table == b->smalltable) {
        ((__builtin_object_size (tab, 0) != (size_t) -1) ? __builtin___memcpy_chk (tab, a->smalltable, sizeof(tab), __builtin_object_size (tab, 0)) : __inline_memcpy_chk (tab, a->smalltable, sizeof(tab)));
        ((__builtin_object_size (a->smalltable, 0) != (size_t) -1) ? __builtin___memcpy_chk (a->smalltable, b->smalltable, sizeof(tab), __builtin_object_size (a->smalltable, 0)) : __inline_memcpy_chk (a->smalltable, b->smalltable, sizeof(tab)));
        ((__builtin_object_size (b->smalltable, 0) != (size_t) -1) ? __builtin___memcpy_chk (b->smalltable, tab, sizeof(tab), __builtin_object_size (b->smalltable, 0)) : __inline_memcpy_chk (b->smalltable, tab, sizeof(tab)));
    }

    if (PyType_IsSubtype((((PyObject*)(a))->ob_type), &PyFrozenSet_Type) &&
        PyType_IsSubtype((((PyObject*)(b))->ob_type), &PyFrozenSet_Type)) {
...
{
    return make_new_set_basetype((((PyObject*)(so))->ob_type), (PyObject *)so);
}

static PyObject *
frozenset_copy(PySetObject *so)
{
    if (((((PyObject*)(so))->ob_type) == &PyFrozenSet_Type)) {
        ( ((PyObject*)(so))->ob_refcnt++);
        return (PyObject *)so;
    }
    return set_copy(so);
...
static char copy_doc[] = "Return a shallow copy of a set.";
...
static char clear_doc[] = "Remove all elements from this set.";
...
set_union(PySetObject *so, PyObject *args)
{
    ...
    for (i=0 ; i<(((PyVarObject*)(args))->ob_size) ; i++) {
        other = (((PyTupleObject *)(args))->ob_item[i]);
...
set_or(PySetObject *so, PyObject *other)
{
    ...
    if (!((((PyObject*)(so))->ob_type) == &PySet_Type || (((PyObject*)(so))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(so))->ob_type), &PyFrozenSet_Type)) || !((((PyObject*)(other))->ob_type) == &PySet_Type || (((PyObject*)(other))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PyFrozenSet_Type)))
...
set_intersection(PySetObject *so, PyObject *other)
{
    ...
    result = (PySetObject *)make_new_set_basetype((((PyObject*)(so))->ob_type), ((void *)0));
    if (result == ((void *)0))
        return ((void *)0);
    ...
    if (((((PyObject*)(other))->ob_type) == &PySet_Type || (((PyObject*)(other))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PyFrozenSet_Type))) {
        ...
        if ((((PySetObject *)(other))->used) > (((PySetObject *)(so))->used)) {
            tmp = (PyObject *)so;
...
static char intersection_update_doc[] = "Update a set with the intersection of itself and another.";
...
set_isdisjoint(PySetObject *so, PyObject *other)
{
    ...
    if (((((PyObject*)(other))->ob_type) == &PySet_Type || (((PyObject*)(other))->ob_type) == &PyFrozenSet_Type)) {
        Py_ssize_t pos = 0;
        setentry *entry;
...
static char isdisjoint_doc[] = "Return True if two sets have a null intersection.";
...
static char difference_update_doc[] = "Remove all elements of another set from this set.";
...
set_difference(PySetObject *so, PyObject *other)
{
    ...
    if (!((((PyObject*)(other))->ob_type) == &PySet_Type || (((PyObject*)(other))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PyFrozenSet_Type)) && !((((PyObject*)(other))->ob_type) == &PyDict_Type)) {
        return set_copy_and_difference(so, other);
    }



    if (((((PySetObject *)(so))->used) >> 2) > PyObject_Size(other)) {
        return set_copy_and_difference(so, other);
    }

    result = make_new_set_basetype((((PyObject*)(so))->ob_type), ((void *)0));
    if (result == ((void *)0))
        return ((void *)0);
...
set_difference_multi(PySetObject *so, PyObject *args)
{
    ...
    for (i=1 ; i<(((PyVarObject*)(args))->ob_size) ; i++) {
        other = (((PyTupleObject *)(args))->ob_item[i]);
        if (set_difference_update_internal((PySetObject *)result, other) == -1) {
            do { if ( --((PyObject*)(result))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(result)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(result)))); } while (0);
            return ((void *)0);
...
set_symmetric_difference_update(PySetObject *so, PyObject *other)
{
    ...
    if (((((PyObject*)(other))->ob_type) == &PySet_Type || (((PyObject*)(other))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(other))->ob_type), &PyFrozenSet_Type))) {
        ...
        otherset = (PySetObject *)make_new_set_basetype((((PyObject*)(so))->ob_type), other);
        if (otherset == ((void *)0))
            return ((void *)0);
    }
    ...
    while (set_next(otherset, &pos, &entry)) {
        ...
        if (rv == -1) {
            do { if ( --((PyObject*)(otherset))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(otherset)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(otherset)))); } while (0);
            return ((void *)0);
        }
        if (rv == 0) {
...
static char symmetric_difference_update_doc[] = "Update a set with the symmetric difference of itself and another.";
...
set_issubset(PySetObject *so, PyObject *other)
{
    ...
    if ((((PySetObject *)(so))->used) > (((PySetObject *)(other))->used))
        return ( ((PyObject*)(((PyObject *) &_Py_FalseStruct)))->ob_refcnt++), ((PyObject *) &_Py_FalseStruct);
...
static char issubset_doc[] = "Report whether another set contains this set.";
...
static char issuperset_doc[] = "Report whether this set contains another set.";
...
set_richcompare(PySetObject *v, PyObject *w, int op)
{
    ...
    if(!((((PyObject*)(w))->ob_type) == &PySet_Type || (((PyObject*)(w))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(w))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(w))->ob_type), &PyFrozenSet_Type)))
        return ( ((PyObject*)((&_Py_NotImplementedStruct)))->ob_refcnt++), (&_Py_NotImplementedStruct);

    switch (op) {
    case 2:
        if ((((PySetObject *)(v))->used) != (((PySetObject *)(w))->used))
            return ( ((PyObject*)(((PyObject *) &_Py_FalseStruct)))->ob_refcnt++), ((PyObject *) &_Py_FalseStruct);
        if (v->hash != -1 &&
    ...
    case 3:
        r1 = set_richcompare(v, w, 2);
        if (r1 == ((void *)0))
            return ((void *)0);
        r2 = PyBool_FromLong(PyObject_Not(r1));
        do { if ( --((PyObject*)(r1))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(r1)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(r1)))); } while (0);
        return r2;
    case 1:
        return set_issubset(v, w);
    case 5:
...
set_contains(PySetObject *so, PyObject *key)
{
    ...
    if (rv == -1) {
        if (!((((PyObject*)(key))->ob_type) == &PySet_Type || PyType_IsSubtype((((PyObject*)(key))->ob_type), &PySet_Type)) || !PyErr_ExceptionMatches(PyExc_TypeError))
            return -1;
        PyErr_Clear();
        ...
        do { if ( --((PyObject*)(tmpkey))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(tmpkey)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(tmpkey)))); } while (0);
    }
    return rv;
}
...
static char contains_doc[] = "x.__contains__(y) <==> y in x.";
...
{
    PyObject *keys=((void *)0), *args=((void *)0), *result=((void *)0), *dict=((void *)0);
    ...
    result = PyTuple_Pack(3, (((PyObject*)(so))->ob_type), args, dict);
done:
...
static char sizeof_doc[] = "S.__sizeof__() -> size of S in memory, in bytes";
static int
set_init(PySetObject *self, PyObject *args, PyObject *kwds)
{
    PyObject *iterable = ((void *)0);

    if (!((((PyObject*)(self))->ob_type) == &PySet_Type || (((PyObject*)(self))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(self))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(self))->ob_type), &PyFrozenSet_Type)))
        return -1;
    if (((((PyObject*)(self))->ob_type) == &PySet_Type || PyType_IsSubtype((((PyObject*)(self))->ob_type), &PySet_Type)) && !_PyArg_NoKeywords("set()", kwds))
        return -1;
    if (!PyArg_UnpackTuple(args, (((PyObject*)(self))->ob_type)->tp_name, 0, 1, &iterable))
        return -1;
...
static PyMethodDef set_methods[] = {
    {"add", (PyCFunction)set_add, 0x0008,
     add_doc},
    {"clear", (PyCFunction)set_clear, 0x0004,
     clear_doc},
    {"__contains__",(PyCFunction)set_direct_contains, 0x0008 | 0x0040,
     contains_doc},
    {"copy", (PyCFunction)set_copy, 0x0004,
     copy_doc},
    {"discard", (PyCFunction)set_discard, 0x0008,
     discard_doc},
    {"difference", (PyCFunction)set_difference_multi, 0x0001,
     difference_doc},
    {"difference_update", (PyCFunction)set_difference_update, 0x0001,
     difference_update_doc},
    {"intersection",(PyCFunction)set_intersection_multi, 0x0001,
     intersection_doc},
    {"intersection_update",(PyCFunction)set_intersection_update_multi, 0x0001,
     intersection_update_doc},
    {"isdisjoint", (PyCFunction)set_isdisjoint, 0x0008,
     isdisjoint_doc},
    {"issubset", (PyCFunction)set_issubset, 0x0008,
     issubset_doc},
    {"issuperset", (PyCFunction)set_issuperset, 0x0008,
     issuperset_doc},
    {"pop", (PyCFunction)set_pop, 0x0004,
     pop_doc},
    {"__reduce__", (PyCFunction)set_reduce, 0x0004,
     reduce_doc},
    {"remove", (PyCFunction)set_remove, 0x0008,
     remove_doc},
    {"__sizeof__", (PyCFunction)set_sizeof, 0x0004,
     sizeof_doc},
    {"symmetric_difference",(PyCFunction)set_symmetric_difference, 0x0008,
     symmetric_difference_doc},
    {"symmetric_difference_update",(PyCFunction)set_symmetric_difference_update, 0x0008,
     symmetric_difference_update_doc},




    {"union", (PyCFunction)set_union, 0x0001,
     union_doc},
    {"update", (PyCFunction)set_update, 0x0001,
     update_doc},
    {((void *)0), ((void *)0)}
};
...
PyTypeObject PySet_Type = {
    ...
    __builtin_offsetof (PySetObject, weakreflist),
...
static PyMethodDef frozenset_methods[] = {
    {"__contains__",(PyCFunction)set_direct_contains, 0x0008 | 0x0040,
     contains_doc},
    {"copy", (PyCFunction)frozenset_copy, 0x0004,
     copy_doc},
    {"difference", (PyCFunction)set_difference_multi, 0x0001,
     difference_doc},
    {"intersection",(PyCFunction)set_intersection_multi, 0x0001,
...
{
    if (!((((PyObject*)(anyset))->ob_type) == &PySet_Type || (((PyObject*)(anyset))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(anyset))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(anyset))->ob_type), &PyFrozenSet_Type))) {
        _PyErr_BadInternalCall("setobject.c", 2317);
        return -1;
    }
...
{
    if (!((((PyObject*)(set))->ob_type) == &PySet_Type || PyType_IsSubtype((((PyObject*)(set))->ob_type), &PySet_Type))) {
        _PyErr_BadInternalCall("setobject.c", 2327);
        return -1;
    }
...
{
    if (!((((PyObject*)(anyset))->ob_type) == &PySet_Type || (((PyObject*)(anyset))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(anyset))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(anyset))->ob_type), &PyFrozenSet_Type))) {
        _PyErr_BadInternalCall("setobject.c", 2337);
        return -1;
    }
...
{
    if (!((((PyObject*)(set))->ob_type) == &PySet_Type || PyType_IsSubtype((((PyObject*)(set))->ob_type), &PySet_Type))) {
        _PyErr_BadInternalCall("setobject.c", 2347);
        return -1;
    }
...
{
    if (!((((PyObject*)(anyset))->ob_type) == &PySet_Type || PyType_IsSubtype((((PyObject*)(anyset))->ob_type), &PySet_Type)) &&
        (!((((PyObject*)(anyset))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(anyset))->ob_type), &PyFrozenSet_Type)) || (((PyObject*)(anyset))->ob_refcnt) != 1)) {
        _PyErr_BadInternalCall("setobject.c", 2358);
        return -1;
    }
...
_PySet_NextEntry(PyObject *set, Py_ssize_t *pos, PyObject **key, Py_hash_t *hash)
{
    ...
    if (!((((PyObject*)(set))->ob_type) == &PySet_Type || (((PyObject*)(set))->ob_type) == &PyFrozenSet_Type || PyType_IsSubtype((((PyObject*)(set))->ob_type), &PySet_Type) || PyType_IsSubtype((((PyObject*)(set))->ob_type), &PyFrozenSet_Type))) {
        _PyErr_BadInternalCall("setobject.c", 2370);
        return -1;
    }
...
{
    if (!((((PyObject*)(set))->ob_type) == &PySet_Type || PyType_IsSubtype((((PyObject*)(set))->ob_type), &PySet_Type))) {
        _PyErr_BadInternalCall("setobject.c", 2384);
        return ((void *)0);
    }
...
{
    if (!((((PyObject*)(set))->ob_type) == &PySet_Type || PyType_IsSubtype((((PyObject*)(set))->ob_type), &PySet_Type))) {
        _PyErr_BadInternalCall("setobject.c", 2394);
        return -1;
    }
