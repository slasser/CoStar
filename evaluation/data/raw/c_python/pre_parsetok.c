...
{
    return PyParser_ParseStringFlagsFilename(s, ((void *)0), g, start, err_ret, 0);
}
...
{
    return PyParser_ParseStringFlagsFilename(s, ((void *)0),
                          ...
                          perrdetail *err_ret, int *flags)
{
    ...
    if (tok == ((void *)0)) {
        err_ret->error = PyErr_Occurred() ? 22 : 15;
        return ((void *)0);
    }


    ( ((PyObject*)(err_ret->filename))->ob_refcnt++);
...
{
    return PyParser_ParseFileFlags(fp, filename, ((void *)0),
                          ...
                          char *ps1, char *ps2, perrdetail *err_ret, int *flags)
{
    ...
    if ((tok = PyTokenizer_FromFile(fp, (char *)enc, ps1, ps2)) == ((void *)0)) {
        err_ret->error = 15;
        return ((void *)0);
    }
         ...
         int *flags)
{
    ...
    if ((ps = PyParser_New(g, start)) == ((void *)0)) {
        fprintf(__stderrp, "no mem for new parser\n");
        err_ret->error = 15;
    ...
    for (;;) {
        ...
        if (str == ((void *)0)) {
            fprintf(__stderrp, "no mem for next token\n");
            err_ret->error = 15;
            break;
        }
        if (len > 0)
            ((__builtin_object_size (str, 0) != (size_t) -1) ? __builtin___strncpy_chk (str, a, len, __builtin_object_size (str, 2 > 1)) : __inline_strncpy_chk (str, a, len));
        str[len] = '\0';


        if (type == 28) {
            if (!(ps->p_flags & 0x40000) &&
                            strcmp(str, "!=")) {
                PyObject_Free(str);
                err_ret->error = 14;
                break;
            }
            else if ((ps->p_flags & 0x40000) &&
                            strcmp(str, "<>")) {
                PyObject_Free(str);
    ...
    *flags = ps->p_flags;
    ...
    if (n == ((void *)0)) {
        ...
        if (tok->buf != ((void *)0)) {
            size_t len;
            (__builtin_expect(!(tok->cur - tok->buf < 2147483647), 0) ? __assert_rtn(__func__, "parsetok.c", 272, "tok->cur - tok->buf < INT_MAX") : (void)0);
            err_ret->offset = (int)(tok->cur - tok->buf);
            len = tok->inp - tok->buf;
            err_ret->text = (char *) PyObject_Malloc(len + 1);
            if (err_ret->text != ((void *)0)) {
                if (len > 0)
                    ((__builtin_object_size (err_ret->text, 0) != (size_t) -1) ? __builtin___strncpy_chk (err_ret->text, tok->buf, len, __builtin_object_size (err_ret->text, 2 > 1)) : __inline_strncpy_chk (err_ret->text, tok->buf, len));
                err_ret->text[len] = '\0';
            }
        }
    } else if (tok->encoding != ((void *)0)) {



        node* r = PyNode_New(335);
        if (r)
        ...
        ((__builtin_object_size (r->n_str, 0) != (size_t) -1) ? __builtin___strcpy_chk (r->n_str, tok->encoding, __builtin_object_size (r->n_str, 2 > 1)) : __inline_strcpy_chk (r->n_str, tok->encoding));
        free(tok->encoding);
        tok->encoding = ((void *)0);
        r->n_nchildren = 1;
        r->n_child = n;
