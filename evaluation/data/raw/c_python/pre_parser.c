...
s_push(register stack *s, dfa *d, node *parent)
{
    ...
    if (s->s_top == s->s_base) {
        fprintf(__stderrp, "s_push: parser stack overflow\n");
        return 15;
    }
    top = --s->s_top;
    top->s_dfa = d;
...
PyParser_New(grammar *g, int start)
{
    ...
    ps = (parser_state *)((size_t)(sizeof(parser_state)) > (size_t)((Py_ssize_t)(((size_t)-1)>>1)) ? ((void *)0) : malloc((sizeof(parser_state)) ? (sizeof(parser_state)) : 1));
    if (ps == ((void *)0))
        return ((void *)0);
    ps->p_grammar = g;
...
{
    int err;
    (__builtin_expect(!(!((s)->s_top == &(s)->s_base[1500])), 0) ? __assert_rtn(__func__, "parser.c", 111, "!s_empty(s)") : (void)0);
...
push(register stack *s, int type, dfa *d, int newstate, int lineno, int col_offset)
{
    ...
    register node *n;
    n = s->s_top->s_parent;
    (__builtin_expect(!(!((s)->s_top == &(s)->s_base[1500])), 0) ? __assert_rtn(__func__, "parser.c", 125, "!s_empty(s)") : (void)0);
    err = PyNode_AddChild(n, type, (char *)((void *)0), lineno, col_offset);
    if (err)
        return err;
    s->s_top->s_state = newstate;
    return s_push(s, d, (&(n)->n_child[((n)->n_nchildren)-1]));
}
...
classify(parser_state *ps, int type, char *str)
{
    ...
    if (type == 1) {
        ...
        register int i;
        for (i = n; i > 0; i--, l++) {
            if (l->lb_type != 1 || l->lb_str == ((void *)0) ||
                l->lb_str[0] != s[0] ||
                strcmp(l->lb_str, s) != 0)
                continue;
# 162 "parser.c"
...

    {
        ...
        register int i;
        for (i = n; i > 0; i--, l++) {
            if (l->lb_type == type && l->lb_str == ((void *)0)) {
                ;
                return n - i;
            }
        }
...
# 292 "parser.c"
                    (&ps->p_stack)->s_top++;
                    if (((&ps->p_stack)->s_top == &(&ps->p_stack)->s_base[1500])) {
                        ;
                        return 16;
                    }
