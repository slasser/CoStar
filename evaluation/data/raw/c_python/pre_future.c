...
future_check_features(PyFutureFeatures *ff, stmt_ty s, const char *filename)
{
    ...
    (__builtin_expect(!(s->kind == ImportFrom_kind), 0) ? __assert_rtn(__func__, "future.c", 19, "s->kind == ImportFrom_kind") : (void)0);

    names = s->v.ImportFrom.names;
    for (i = 0; i < ((names) == ((void *)0) ? 0 : (names)->size); i++) {
        alias_ty name = (alias_ty)(names)->elements[(i)];
        const char *feature = PyUnicode_AsUTF8(name->name);
        if (!feature)
            return 0;
        if (strcmp(feature, "nested_scopes") == 0) {
            continue;
        } else if (strcmp(feature, "generators") == 0) {
            continue;
        } else if (strcmp(feature, "division") == 0) {
            continue;
        } else if (strcmp(feature, "absolute_import") == 0) {
            continue;
        } else if (strcmp(feature, "with_statement") == 0) {
            continue;
        } else if (strcmp(feature, "print_function") == 0) {
            continue;
        } else if (strcmp(feature, "unicode_literals") == 0) {
            continue;
        } else if (strcmp(feature, "barry_as_FLUFL") == 0) {
            ff->ff_features |= 0x40000;
            ...
            PyErr_Format(PyExc_SyntaxError,
                         "future feature %.100s is not defined", feature);
            PyErr_SyntaxLocationEx(filename, s->lineno, s->col_offset);
            return 0;
        }
    }
...
future_parse(PyFutureFeatures *ff, mod_ty mod, const char *filename)
{
    ...
    if (!(mod->kind == Module_kind || mod->kind == Interactive_kind))
        return 1;
# 75 "future.c"
    for (i = 0; i < ((mod->v.Module.body) == ((void *)0) ? 0 : (mod->v.Module.body)->size); i++) {
        stmt_ty s = (stmt_ty)(mod->v.Module.body)->elements[(i)];
