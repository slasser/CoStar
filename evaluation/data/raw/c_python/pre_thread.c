...
# 14 "thread_pthread.h" 2
# 138 "thread_pthread.h"
typedef struct {
    char locked;
...
PyThread_allocate_lock(void)
{
    ...
    if (lock) {
        ((__builtin_object_size ((void *)lock, 0) != (size_t) -1) ? __builtin___memset_chk ((void *)lock, '\0', sizeof(pthread_lock), __builtin_object_size ((void *)lock, 0)) : __inline_memset_chk ((void *)lock, '\0', sizeof(pthread_lock)));
        lock->locked = 0;

        status = pthread_mutex_init(&lock->mut,
                                    ((pthread_mutexattr_t *)((void *)0)));
        if (status != 0) { perror("pthread_mutex_init"); error = 1; };
        ...
        status = pthread_cond_init(&lock->lock_released,
                                   ((pthread_condattr_t *)((void *)0)));
        if (status != 0) { perror("pthread_cond_init"); error = 1; };
...
PyThread_free_lock(PyThread_type_lock lock)
{
    ...
    if (status != 0) { perror("pthread_cond_destroy"); error = 1; };

    status = pthread_mutex_destroy( &thelock->mut );
    if (status != 0) { perror("pthread_mutex_destroy"); error = 1; };
                            ...
                            int intr_flag)
{
    ...
    if (status != 0) { perror("pthread_mutex_lock[1]"); error = 1; };
    ...
    if (thelock->locked == 0) {
        ...
        struct timespec ts;
        if (microseconds > 0)
            do { struct timeval tv; gettimeofday(&tv, (struct timezone *)((void *)0)); tv.tv_usec += microseconds % 1000000; tv.tv_sec += microseconds / 1000000; tv.tv_sec += tv.tv_usec / 1000000; tv.tv_usec %= 1000000; ts.tv_sec = tv.tv_sec; ts.tv_nsec = tv.tv_usec * 1000; } while(0);
        ...
        while (success == PY_LOCK_FAILURE) {
            if (microseconds > 0) {
                ...
                if (status == 60)
                    break;
                if (status != 0) { perror("pthread_cond_timed_wait"); error = 1; };
            }
            else {
                status = pthread_cond_wait(
                    &thelock->lock_released,
                    &thelock->mut);
                if (status != 0) { perror("pthread_cond_wait"); error = 1; };
            }
    ...
    if (status != 0) { perror("pthread_mutex_unlock[1]"); error = 1; };
...
PyThread_release_lock(PyThread_type_lock lock)
{
    ...
    status = pthread_mutex_lock( &thelock->mut );
    if (status != 0) { perror("pthread_mutex_lock[3]"); error = 1; };

    thelock->locked = 0;


    status = pthread_cond_signal( &thelock->lock_released );
    if (status != 0) { perror("pthread_cond_signal"); error = 1; };

    status = pthread_mutex_unlock( &thelock->mut );
    if (status != 0) { perror("pthread_mutex_unlock[3]"); error = 1; };
}
...
{
    pthread_key_t key;
    int fail = pthread_key_create(&key, ((void *)0));
    return fail ? -1 : key;
}
...
PyThread_set_stacksize(size_t size)
{
    ...
    return _pythread_pthread_set_stacksize(size);



}
# 374 "thread.c"
static char threadinfo__doc__[] = "sys.thread_info\n\nA struct sequence holding information about the thread implementation.";
...
PyThread_GetInfo(void)
{
    ...
    if (threadinfo == ((void *)0))
        return ((void *)0);

    value = PyUnicode_FromString("pthread");
    if (value == ((void *)0)) {
        do { if ( --((PyObject*)(threadinfo))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(threadinfo)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(threadinfo)))); } while (0);
        return ((void *)0);
    }
    (((PyTupleObject *)(threadinfo))->ob_item[pos++] = value);
