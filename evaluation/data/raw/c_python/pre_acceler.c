...
fixstate(grammar *g, state *s)
{
    ...
    if (accel == ((void *)0)) {
        fprintf(__stderrp, "no mem to build parser accelerators\n");
        exit(1);
    }
    for (k = 0; k < nl; k++)
        accel[k] = -1;
    ...
    for (k = s->s_narcs; --k >= 0; a++) {
        ...
        if (((type) >= 256)) {
            ...
            for (ibit = 0; ibit < g->g_ll.ll_nlabels; ibit++) {
                if ((((d1->d_first)[((ibit) / (8*sizeof(char)))] & (1 << ((ibit) % (8*sizeof(char))))) != 0)) {
                    if (accel[ibit] != -1)
    ...
    if (k < nl) {
        ...
        if (s->s_accel == ((void *)0)) {
            fprintf(__stderrp, "no mem to add parser accelerators\n");
            exit(1);
        }
        s->s_lower = k;
        s->s_upper = nl;
