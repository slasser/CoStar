...
calcfirstset(grammar *g, dfa *d)
{
    ...
    if (dummy == ((void *)0))
        dummy = _Py_newbitset(1);
    if (d->d_first == dummy) {
        fprintf(__stderrp, "Left-recursion for '%s'\n", d->d_name);
        return;
    }
    if (d->d_first != ((void *)0)) {
        fprintf(__stderrp, "Re-calculating FIRST set for '%s' ???\n",
            d->d_name);
    }
    d->d_first = dummy;
        ...
        if (j >= nsyms) {
            ...
            if (((type) >= 256)) {
                ...
                if (d1->d_first == dummy) {
                    ...
                    if (d1->d_first == ((void *)0))
                        calcfirstset(g, d1);
                    _Py_mergebitset(result,
                                d1->d_first, nbits);
                }
            }
            else if (((type) < 256)) {
                _Py_addbit(result, a->a_lbl);
            }
        }
    }
    d->d_first = result;
    if (Py_DebugFlag) {
        ...
        for (i = 0; i < nbits; i++) {
            if ((((result)[((i) / (8*sizeof(char)))] & (1 << ((i) % (8*sizeof(char))))) != 0))
