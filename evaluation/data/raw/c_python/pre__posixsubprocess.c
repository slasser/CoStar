...
# 19 "_posixsubprocess.c" 2
# 42 "_posixsubprocess.c"
static long max_fd;
...
{
    PyObject *result;
    static _Py_Identifier PyId_enable = { 0, "enable", 0 };

    result = _PyObject_CallMethodId(gc_module, &PyId_enable, ((void *)0));
    if (result == ((void *)0))
        return 1;
...
# 99 "_posixsubprocess.c"
static int
_sanity_check_python_fd_sequence(PyObject *fd_sequence)
{
    Py_ssize_t seq_idx, seq_len = PySequence_Size(fd_sequence);
    long prev_fd = -1;
    for (seq_idx = 0; seq_idx < seq_len; ++seq_idx) {
        PyObject* py_fd = (((((((PyObject*)(fd_sequence))->ob_type))->tp_flags & ((1L<<25))) != 0) ? (((PyListObject *)(fd_sequence))->ob_item[seq_idx]) : (((PyTupleObject *)(fd_sequence))->ob_item[seq_idx]));
        long iter_fd = PyLong_AsLong(py_fd);
        if (iter_fd < 0 || iter_fd < prev_fd || iter_fd > 2147483647) {
...
_is_fd_in_sorted_fd_sequence(int fd, PyObject *fd_sequence)
{
    ...
    Py_ssize_t search_max = PySequence_Size(fd_sequence) - 1;
    if (search_max < 0)
        return 0;
    do {
        long middle = (search_min + search_max) / 2;
        long middle_fd = PyLong_AsLong(
                (((((((PyObject*)(fd_sequence))->ob_type))->tp_flags & ((1L<<25))) != 0) ? (((PyListObject *)(fd_sequence))->ob_item[middle]) : (((PyTupleObject *)(fd_sequence))->ob_item[middle])));
        if (fd == middle_fd)
            return 1;
        if (fd > middle_fd)
            search_min = middle + 1;
...
{
    Py_ssize_t num_fds_to_keep = PySequence_Size(py_fds_to_keep);
    Py_ssize_t keep_seq_idx;
    int fd_num;


    for (keep_seq_idx = 0; keep_seq_idx < num_fds_to_keep; ++keep_seq_idx) {
        PyObject* py_keep_fd = (((((((PyObject*)(py_fds_to_keep))->ob_type))->tp_flags & ((1L<<25))) != 0) ? (((PyListObject *)(py_fds_to_keep))->ob_item[keep_seq_idx]) : (((PyTupleObject *)(py_fds_to_keep))->ob_item[keep_seq_idx]));

        int keep_fd = PyLong_AsLong(py_keep_fd);
        if (keep_fd < start_fd)
            continue;
        for (fd_num = start_fd; fd_num < keep_fd; ++fd_num) {
            while (close(fd_num) < 0 && (*__error()) == 4);
        }
        start_fd = keep_fd + 1;
    }
    if (start_fd <= end_fd) {
...
# 284 "_posixsubprocess.c"
    ...
    if (!proc_fd_dir) {
        ...
        int fd_used_by_opendir = ((proc_fd_dir)->__dd_fd);
           ...
           PyObject *preexec_fn_args_tuple)
{
    ...
    if (p2cwrite != -1) {
        if ((close(p2cwrite)) == -1) goto error;
    }
    if (c2pread != -1) {
        if ((close(c2pread)) == -1) goto error;
    }
    if (errread != -1) {
        if ((close(errread)) == -1) goto error;
    }
    if ((close(errpipe_read)) == -1) goto error;



    if (c2pwrite == 0)
        if ((c2pwrite = dup(c2pwrite)) == -1) goto error;
    if (errwrite == 0 || errwrite == 1)
        if ((errwrite = dup(errwrite)) == -1) goto error;




    if (p2cread == 0) {
        int old = fcntl(p2cread, 1);
        if (old != -1)
            fcntl(p2cread, 2, old & ~1);
    } else if (p2cread != -1) {
        if ((dup2(p2cread, 0)) == -1) goto error;
    }
    if (c2pwrite == 1) {
        int old = fcntl(c2pwrite, 1);
        if (old != -1)
            fcntl(c2pwrite, 2, old & ~1);
    } else if (c2pwrite != -1) {
        if ((dup2(c2pwrite, 1)) == -1) goto error;
    }
    if (errwrite == 2) {
        int old = fcntl(errwrite, 1);
        if (old != -1)
            fcntl(errwrite, 2, old & ~1);
    } else if (errwrite != -1) {
        if ((dup2(errwrite, 2)) == -1) goto error;
    }



    if (p2cread > 2) {
        if ((close(p2cread)) == -1) goto error;
    }
    if (c2pwrite > 2 && c2pwrite != p2cread) {
        if ((close(c2pwrite)) == -1) goto error;
    }
    if (errwrite != c2pwrite && errwrite != p2cread && errwrite > 2) {
        if ((close(errwrite)) == -1) goto error;
    }
    ...
    if (close_fds) {
        ...
        _close_open_fd_range_maybe_unsafe(3, local_max_fd, py_fds_to_keep);
    }
    ...
    reached_preexec = 1;
    if (preexec_fn != (&_Py_NoneStruct) && preexec_fn_args_tuple) {

        result = PyObject_Call(preexec_fn, preexec_fn_args_tuple, ((void *)0));
        if (result == ((void *)0)) {
    ...
    for (i = 0; exec_array[i] != ((void *)0); ++i) {
        ...
        if ((*__error()) != 2 && (*__error()) != 20 && saved_errno == 0) {
            saved_errno = (*__error());
        }
    }
...
{
    PyObject *gc_module = ((void *)0);
    PyObject *executable_list, *py_fds_to_keep;
    PyObject *env_list, *preexec_fn;
    PyObject *process_args, *converted_args = ((void *)0), *fast_args = ((void *)0);
    PyObject *preexec_fn_args_tuple = ((void *)0);
    ...
    char *const *exec_array, *const *argv = ((void *)0), *const *envp = ((void *)0);
    Py_ssize_t arg_num;
    ...
    if (PySequence_Size(py_fds_to_keep) < 0) {
    ...
    if (preexec_fn != (&_Py_NoneStruct)) {
        PyObject *result;
        static _Py_Identifier PyId_isenabled = { 0, "isenabled", 0 };
        static _Py_Identifier PyId_disable = { 0, "disable", 0 };

        gc_module = PyImport_ImportModule("gc");
        if (gc_module == ((void *)0))
            return ((void *)0);
        result = _PyObject_CallMethodId(gc_module, &PyId_isenabled, ((void *)0));
        if (result == ((void *)0)) {
            do { if ( --((PyObject*)(gc_module))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(gc_module)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(gc_module)))); } while (0);
            return ((void *)0);
        }
        ...
        result = _PyObject_CallMethodId(gc_module, &PyId_disable, ((void *)0));
        if (result == ((void *)0)) {
            do { if ( --((PyObject*)(gc_module))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(gc_module)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(gc_module)))); } while (0);
            return ((void *)0);
        }
    ...
    if (!exec_array) {
        do { if ((gc_module) == ((void *)0)) ; else do { if ( --((PyObject*)(gc_module))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(gc_module)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(gc_module)))); } while (0); } while (0);
        return ((void *)0);
    }
    ...
    if (process_args != (&_Py_NoneStruct)) {
        Py_ssize_t num_args;
        ...
        if (fast_args == ((void *)0))
            goto cleanup;
        num_args = (((((((PyObject*)(fast_args))->ob_type))->tp_flags & ((1L<<25))) != 0) ? (((PyVarObject*)(fast_args))->ob_size) : (((PyVarObject*)(fast_args))->ob_size));
        converted_args = PyTuple_New(num_args);
        if (converted_args == ((void *)0))
            goto cleanup;
        for (arg_num = 0; arg_num < num_args; ++arg_num) {
            PyObject *borrowed_arg, *converted_arg;
            borrowed_arg = (((((((PyObject*)(fast_args))->ob_type))->tp_flags & ((1L<<25))) != 0) ? (((PyListObject *)(fast_args))->ob_item[arg_num]) : (((PyTupleObject *)(fast_args))->ob_item[arg_num]));
            if (PyUnicode_FSConverter(borrowed_arg, &converted_arg) == 0)
                goto cleanup;
            (((PyTupleObject *)(converted_args))->ob_item[arg_num] = converted_arg);
        }

        argv = _PySequence_BytesToCharpArray(converted_args);
        do { if (converted_args) { PyObject *_py_tmp = (PyObject *)(converted_args); (converted_args) = ((void *)0); do { if ( --((PyObject*)(_py_tmp))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(_py_tmp)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(_py_tmp)))); } while (0); } } while (0);
        do { if (fast_args) { PyObject *_py_tmp = (PyObject *)(fast_args); (fast_args) = ((void *)0); do { if ( --((PyObject*)(_py_tmp))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(_py_tmp)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(_py_tmp)))); } while (0); } } while (0);
        if (!argv)
            goto cleanup;
    }

    if (env_list != (&_Py_NoneStruct)) {
    ...
    do { if ((cwd_obj2) == ((void *)0)) ; else do { if ( --((PyObject*)(cwd_obj2))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(cwd_obj2)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(cwd_obj2)))); } while (0); } while (0);
    ...
    do { if ((preexec_fn_args_tuple) == ((void *)0)) ; else do { if ( --((PyObject*)(preexec_fn_args_tuple))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(preexec_fn_args_tuple)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(preexec_fn_args_tuple)))); } while (0); } while (0);
    do { if ((gc_module) == ((void *)0)) ; else do { if ( --((PyObject*)(gc_module))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(gc_module)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(gc_module)))); } while (0); } while (0);
...
cleanup:
    ...
    if (argv)
        _Py_FreeCharPArray(argv);
    _Py_FreeCharPArray(exec_array);
    do { if ((converted_args) == ((void *)0)) ; else do { if ( --((PyObject*)(converted_args))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(converted_args)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(converted_args)))); } while (0); } while (0);
    do { if ((fast_args) == ((void *)0)) ; else do { if ( --((PyObject*)(fast_args))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(fast_args)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(fast_args)))); } while (0); } while (0);
    do { if ((preexec_fn_args_tuple) == ((void *)0)) ; else do { if ( --((PyObject*)(preexec_fn_args_tuple))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(preexec_fn_args_tuple)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(preexec_fn_args_tuple)))); } while (0); } while (0);
...
static char subprocess_fork_exec_doc[] = "fork_exec(args, executable_list, close_fds, cwd, env,\n          p2cread, p2cwrite, c2pread, c2pwrite,\n          errread, errwrite, errpipe_read, errpipe_write,\n          restore_signals, call_setsid, preexec_fn)\n\nForks a child process, closes parent file descriptors as appropriate in the\nchild and dups the few that are needed before calling exec() in the child\nprocess.\n\nThe preexec_fn, if supplied, will be called immediately before exec.\nWARNING: preexec_fn is NOT SAFE if your application uses threads.\n         It may trigger infrequent, difficult to debug deadlocks.\n\nIf an error occurs in the child process before the exec, it is\nserialized and written to the errpipe_write fd per subprocess.py.\n\nReturns: the child process's PID.\n\nRaises: Only on an error in the parent process.\n";
# 723 "_posixsubprocess.c"
static char subprocess_cloexec_pipe_doc[] = "cloexec_pipe() -> (read_end, write_end)\n\nCreate a pipe whose ends have the cloexec flag set.";
...
subprocess_cloexec_pipe(PyObject *self, PyObject *noargs)
{
    int fds[2];
    int res;
# 743 "_posixsubprocess.c"
        long oldflags;

        res = pipe(fds);

        if (res == 0) {
            oldflags = fcntl(fds[0], 1, 0);
            if (oldflags < 0) res = oldflags;
        }
        if (res == 0)
            res = fcntl(fds[0], 2, oldflags | 1);

        if (res == 0) {
            oldflags = fcntl(fds[1], 1, 0);
            if (oldflags < 0) res = oldflags;
        }
        if (res == 0)
            res = fcntl(fds[1], 2, oldflags | 1);
...
static char module_doc[] = "A POSIX helper for the subprocess module.";



static PyMethodDef module_methods[] = {
    {"fork_exec", subprocess_fork_exec, 0x0001, subprocess_fork_exec_doc},
    {"cloexec_pipe", subprocess_cloexec_pipe, 0x0004, subprocess_cloexec_pipe_doc},
    {((void *)0), ((void *)0)}
};
...
PyInit__posixsubprocess(void)
{
    ...
    return PyModule_Create2(&_posixsubprocessmodule, 1013);
}
