...
bitset
_Py_newbitset(int nbits)
{
    int nbytes = (((nbits) + (8*sizeof(char)) - 1) / (8*sizeof(char)));
    bitset ss = (char *)PyObject_Malloc(sizeof(char) * nbytes);
...
void
_Py_delbitset(bitset ss)
{
    PyObject_Free(ss);
}

int
_Py_addbit(bitset ss, int ibit)
{
    int ibyte = ((ibit) / (8*sizeof(char)));
    char mask = (1 << ((ibit) % (8*sizeof(char))));
...
# 48 "bitset.c"
int
_Py_samebitset(bitset ss1, bitset ss2, int nbits)
{
    int i;

    for (i = (((nbits) + (8*sizeof(char)) - 1) / (8*sizeof(char))); --i >= 0; )
        if (*ss1++ != *ss2++)
            return 0;
    return 1;
}
...
void
_Py_mergebitset(bitset ss1, bitset ss2, int nbits)
{
    int i;
