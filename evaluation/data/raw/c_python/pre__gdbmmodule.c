...
# 12 "_gdbmmodule.c" 2
# 1 "/usr/local/include/gdbm.h" 1 3
# 75 "/usr/local/include/gdbm.h" 3
typedef struct {
 char *dptr;
 int dsize;
      } datum;
...
# 100 "/usr/local/include/gdbm.h" 3
...
static char gdbmmodule__doc__[] = "This module provides an interface to the GNU DBM (GDBM) library.\n\nThis module is quite similar to the dbm module, but uses GDBM instead to\nprovide some additional functionality. Please note that the file formats\ncreated by GDBM and dbm are incompatible. \n\nGDBM objects behave like mappings (dictionaries), except that keys and\nvalues are always strings. Printing a GDBM object doesn't print the\nkeys and values, and the items() and values() methods are not\nsupported.";
# 31 "_gdbmmodule.c"
typedef struct {
    PyObject ob_base;
    int di_size;
...
static char gdbm_object__doc__[] = "This object represents a GDBM database.\nGDBM objects behave like mappings (dictionaries), except that keys and\nvalues are always strings. Printing a GDBM object doesn't print the\nkeys and values, and the items() and values() methods are not\nsupported.\n\nGDBM objects also support additional operations such as firstkey,\nnextkey, reorganize, and sync.";
# 58 "_gdbmmodule.c"
static PyObject *
newdbmobject(char *file, int flags, int mode)
{
    ...
    dp = ( (dbmobject *) _PyObject_New(&Dbmtype) );
    if (dp == ((void *)0))
        return ((void *)0);
    dp->di_size = -1;
    (*__error()) = 0;
    if ((dp->di_dbm = gdbm_open(file, 0, flags, mode, ((void *)0))) == 0) {
        if ((*__error()) != 0)
            PyErr_SetFromErrno(DbmError);
        else
            PyErr_SetString(DbmError, gdbm_strerror(gdbm_errno));
        do { if ( --((PyObject*)(dp))->ob_refcnt != 0) ; else ( (*(((PyObject*)((PyObject *)(dp)))->ob_type)->tp_dealloc)((PyObject *)((PyObject *)(dp)))); } while (0);
        return ((void *)0);
    }
    return (PyObject *)dp;
}
...
dbm_get(dbmobject *dp, PyObject *args)
{
    ...
    if (res == ((void *)0) && PyErr_ExceptionMatches(PyExc_KeyError)) {
        PyErr_Clear();
...
dbm_ass_sub(dbmobject *dp, PyObject *v, PyObject *w)
{
    ...
    if (w == ((void *)0)) {
        ...
        (*__error()) = 0;
        if (gdbm_store(dp->di_dbm, krec, drec, 1) < 0) {
            if ((*__error()) != 0)
                PyErr_SetFromErrno(DbmError);
            else
                PyErr_SetString(DbmError,
...
static char dbm_setdefault__doc__[] = "setdefault(key[, default]) -> value\nGet value for key, or set it to default and return default if not present;\nif not given, default is None.";
...
static PyMappingMethods dbm_as_mapping = {
    (lenfunc)dbm_length,
    (binaryfunc)dbm_subscript,
    (objobjargproc)dbm_ass_sub,
};

static char dbm_close__doc__[] = "close() -> None\nCloses the database.";
...
static char dbm_keys__doc__[] = "keys() -> list_of_keys\nGet a list of all keys in the database.";
...
dbm_keys(register dbmobject *dp, PyObject *unused)
{
    ...
    if (dp == ((void *)0) || !((((PyObject*)(dp))->ob_type) == &Dbmtype)) {
        _PyErr_BadInternalCall("_gdbmmodule.c", 257);
        return ((void *)0);
    }
    if ((dp)->di_dbm == ((void *)0)) { PyErr_SetString(DbmError, "GDBM object has already been closed"); return ((void *)0); };
...
dbm_contains(PyObject *self, PyObject *arg)
{
    ...
    key.dptr = ((__builtin_expect(!(((((((PyObject*)(arg))->ob_type))->tp_flags & ((1L<<27))) != 0)), 0) ? __assert_rtn(__func__, "_gdbmmodule.c", 305, "PyBytes_Check(arg)") : (void)0), (((PyBytesObject *)(arg))->ob_sval));
    key.dsize = ((__builtin_expect(!(((((((PyObject*)(arg))->ob_type))->tp_flags & ((1L<<27))) != 0)), 0) ? __assert_rtn(__func__, "_gdbmmodule.c", 306, "PyBytes_Check(arg)") : (void)0),(((PyVarObject*)(arg))->ob_size));
...
static char dbm_nextkey__doc__[] = "nextkey(key) -> next_key\nReturns the key that follows key in the traversal.\nThe following code prints every key in the database db, without having\nto create a list in memory that contains them all:\n\n      k = db.firstkey()\n      while k != None:\n          print k\n          k = db.nextkey(k)";
# 360 "_gdbmmodule.c"
static PyObject *
...
static char dbm_sync__doc__[] = "sync() -> None\nWhen the database has been opened in fast mode, this method forces\nany unwritten data to be written to the disk.";
...
static PyMethodDef dbm_methods[] = {
    {"close", (PyCFunction)dbm_close, 0x0004, dbm_close__doc__},
    {"keys", (PyCFunction)dbm_keys, 0x0004, dbm_keys__doc__},
    {"firstkey", (PyCFunction)dbm_firstkey,0x0004, dbm_firstkey__doc__},
    {"nextkey", (PyCFunction)dbm_nextkey, 0x0001, dbm_nextkey__doc__},
    {"reorganize",(PyCFunction)dbm_reorganize,0x0004, dbm_reorganize__doc__},
    {"sync", (PyCFunction)dbm_sync, 0x0004, dbm_sync__doc__},
    {"get", (PyCFunction)dbm_get, 0x0001, dbm_get__doc__},
    {"setdefault",(PyCFunction)dbm_setdefault,0x0001, dbm_setdefault__doc__},
    {((void *)0), ((void *)0)}
};
...
static PyMethodDef dbmmodule_methods[] = {
    { "open", (PyCFunction)dbmopen, 0x0001, dbmopen__doc__},
    { 0, 0 },
};
...
PyInit__gdbm(void) {
    ...
    if (PyType_Ready(&Dbmtype) < 0)
            return ((void *)0);
    m = PyModule_Create2(&_gdbmmodule, 1013);
    if (m == ((void *)0))
        return ((void *)0);
    d = PyModule_GetDict(m);
    DbmError = PyErr_NewException("_gdbm.error", PyExc_IOError, ((void *)0));
    if (DbmError != ((void *)0)) {
