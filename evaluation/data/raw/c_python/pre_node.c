...
fancy_roundup(int n)
{
    ...
    int result = 256;
    (__builtin_expect(!(n > 128), 0) ? __assert_rtn(__func__, "node.c", 27, "n > 128") : (void)0);
    while (result < n) {
        result <<= 1;
        if (result <= 0)
            return -1;
...
PyNode_AddChild(register node *n1, int type, char *str, int lineno, int col_offset)
{
    ...
    if (nch == 2147483647 || nch < 0)
        return 19;

    current_capacity = ((nch) <= 1 ? (nch) : (nch) <= 128 ? (((size_t)((nch)) + (size_t)((4) - 1)) & ~(size_t)((4) - 1)) : fancy_roundup(nch));
    required_capacity = ((nch + 1) <= 1 ? (nch + 1) : (nch + 1) <= 128 ? (((size_t)((nch + 1)) + (size_t)((4) - 1)) & ~(size_t)((4) - 1)) : fancy_roundup(nch + 1));
    if (current_capacity < 0 || required_capacity < 0)
        return 19;
    if (current_capacity < required_capacity) {
        if (required_capacity > 18446744073709551615ULL / sizeof(node)) {
            return 15;
        }
        n = n1->n_child;
...
sizeofchildren(node *n)
{
    ...
    int i;
    for (i = ((n)->n_nchildren); --i >= 0; )
        res += sizeofchildren((&(n)->n_child[i]));
    if (n->n_child != ((void *)0))

        res += ((((n)->n_nchildren)) <= 1 ? (((n)->n_nchildren)) : (((n)->n_nchildren)) <= 128 ? (((size_t)((((n)->n_nchildren))) + (size_t)((4) - 1)) & ~(size_t)((4) - 1)) : fancy_roundup(((n)->n_nchildren))) * sizeof(node);
    if (((n)->n_str) != ((void *)0))
        res += strlen(((n)->n_str)) + 1;
    return res;
}
