...
asdl_seq_new(Py_ssize_t size, PyArena *arena)
{
    ...
    if (size < 0 || size == (-2147483647 - 1) ||
        (size && ((size - 1) > (18446744073709551615ULL / sizeof(void *))))) {
        PyErr_NoMemory();
        return ((void *)0);
    }


    if (n > 18446744073709551615ULL - sizeof(asdl_seq)) {
        PyErr_NoMemory();
        return ((void *)0);
    }
    ...
    ((__builtin_object_size (seq, 0) != (size_t) -1) ? __builtin___memset_chk (seq, 0, n, __builtin_object_size (seq, 0)) : __inline_memset_chk (seq, 0, n));
    seq->size = size;
    return seq;
}
