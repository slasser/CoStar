...
getgrammar(char *filename)
{
    ...
    n = PyParser_ParseFile(fp, filename, g0, g0->g_start,
                  (char *)((void *)0), (char *)((void *)0), &err);
    fclose(fp);
    if (n == ((void *)0)) {
        fprintf(__stderrp, "Parsing error %d, line %d.\n",
            err.error, err.lineno);
        if (err.text != ((void *)0)) {
            size_t i;
            fprintf(__stderrp, "%s", err.text);
            i = strlen(err.text);
            if (i == 0 || err.text[i-1] != '\n')
                fprintf(__stderrp, "\n");
...
{
    fprintf(__stderrp, "pgen: FATAL ERROR: %s\n", msg);
    Py_Exit(1);
}
...
{
    size_t n = 1000;
    char *p = (char *)((size_t)(n) > (size_t)((Py_ssize_t)(((size_t)-1)>>1)) ? ((void *)0) : malloc((n) ? (n) : 1));
    char *q;
    if (p == ((void *)0))
        return ((void *)0);
    fprintf(__stderrp, "%s", prompt);
    ...
    if (n > 0 && p[n-1] != '\n')
        p[n-1] = '\n';
    return (char *)((size_t)(n+1) > (size_t)((Py_ssize_t)(((size_t)-1)>>1)) ? ((void *)0) : realloc((p), (n+1) ? (n+1) : 1));
}
