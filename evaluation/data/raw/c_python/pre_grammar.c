...
_Py_newgrammar(int start)
{
    ...
    g->g_start = start;
    g->g_ll.ll_nlabels = 0;
    g->g_ll.ll_label = ((void *)0);
    g->g_accel = 0;
    return g;
}

dfa *
_Py_adddfa(grammar *g, int type, char *name)
{
    dfa *d;
...
void
_Py_addarc(dfa *d, int from, int to, int lbl)
{
    state *s;
    arc *a;

    (__builtin_expect(!(0 <= from && from < d->d_nstates), 0) ? __assert_rtn(__func__, "grammar.c", 75, "0 <= from && from < d->d_nstates") : (void)0);
    (__builtin_expect(!(0 <= to && to < d->d_nstates), 0) ? __assert_rtn(__func__, "grammar.c", 76, "0 <= to && to < d->d_nstates") : (void)0);
...
int
_Py_addlabel(labellist *ll, int type, char *str)
{
    int i;
    label *lb;
...
int
_Py_findlabel(labellist *ll, int type, char *str)
{
    int i;
    ...
    fprintf(__stderrp, "Label %d/'%s' not found\n", type, str);
...
_Py_translatelabels(grammar *g)
{
    ...
    for (i = 0 +1; i < g->g_ll.ll_nlabels; i++)
...
translabel(grammar *g, label *lb)
{
    ...
    if (lb->lb_type == 3) {
        if (iswalpha(btowc(((unsigned char)((lb->lb_str[1]) & 0xff)))) ||
            lb->lb_str[1] == '_') {
            char *p;
            char *src;
            char *dest;
            ...
            ((__builtin_object_size (dest, 0) != (size_t) -1) ? __builtin___strncpy_chk (dest, src, name_len, __builtin_object_size (dest, 2 > 1)) : __inline_strncpy_chk (dest, src, name_len));
            dest[name_len] = '\0';
            free(lb->lb_str);
            lb->lb_str = dest;
        }
