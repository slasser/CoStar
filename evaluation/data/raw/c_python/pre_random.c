...
dev_urandom_noraise(char *buffer, Py_ssize_t size)
{
    ...
    (__builtin_expect(!(0 < size), 0) ? __assert_rtn(__func__, "random.c", 129, "0 < size") : (void)0);

    fd = open("/dev/urandom", 0x0000);
    if (fd < 0)
...
_PyRandom_Init(void)
{
    ...
    env = (Py_IgnoreEnvironmentFlag ? ((void *)0) : getenv("PYTHONHASHSEED"));
    if (env && *env != '\0' && strcmp(env, "random") != 0) {
        ...
        if (*endptr != '\0'
            || seed > 4294967295UL
            || ((*__error()) == 34 && seed == (9223372036854775807L * 2UL + 1UL)))
        {
        ...
        if (seed == 0) {
            ...
            ((__builtin_object_size (secret, 0) != (size_t) -1) ? __builtin___memset_chk (secret, 0, secret_size, __builtin_object_size (secret, 0)) : __inline_memset_chk (secret, 0, secret_size));
        }
        else {
