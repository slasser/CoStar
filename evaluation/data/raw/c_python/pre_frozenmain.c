...
Py_FrozenMain(int argc, char **argv)
{
    ...
    if ((p = (Py_IgnoreEnvironmentFlag ? ((void *)0) : getenv("PYTHONINSPECT"))) && *p != '\0')
        inspect = 1;
    if ((p = (Py_IgnoreEnvironmentFlag ? ((void *)0) : getenv("PYTHONUNBUFFERED"))) && *p != '\0')
        unbuffered = 1;

    if (unbuffered) {
        setbuf(__stdinp, (char *)((void *)0));
        setbuf(__stdoutp, (char *)((void *)0));
        setbuf(__stderrp, (char *)((void *)0));
    }
    ...
    oldloc = setlocale(0, ((void *)0));
    setlocale(0, "");
    for (i = 0; i < argc; i++) {



        size_t argsize = mbstowcs(((void *)0), argv[i], 0);

        size_t count;
        if (argsize == (size_t)-1) {
            fprintf(__stderrp, "Could not convert argument %d to string\n", i);
            return 1;
        }
    ...
    if (inspect && isatty((int)fileno(__stdinp)))
        sts = PyRun_AnyFileExFlags(__stdinp, "<stdin>", 0, ((void *)0)) != 0;
