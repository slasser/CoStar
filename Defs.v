...
Module DefsFn (Export Ty : SYMBOL_TYPES).
  ...
  Fixpoint grammarOfEntryList (es : list grammar_entry) : grammar :=
    match es with
    | []       => PM.empty grammar_entry
    | e :: es' =>
      match e with
      | @existT _ _ p _ => PM.add p e (grammarOfEntryList es')
      end
    end.

  Lemma grammarOfEntryList_wf :
    forall es,
      grammar_wf (grammarOfEntryList es).
  Proof.
    intros es; induction es as [| [p fs] es IH]; sis; red.
    - intros p p' fs hm.
      exfalso. eapply PMF.empty_mapsto_iff; eauto.
    - intros p' p'' fs'' hm.
      destruct (Production_as_UOT.eq_dec p' p); subst.
